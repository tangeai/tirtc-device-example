...
def redact_rx(text: str) -> str:
    ...
    elif text.startswith("+TIRTC:字幕,"):
        fields = parse_csv_urc(text, "+TIRTC:字幕,")
        text = (
            replace_csv_fields(
                text,
                "+TIRTC:字幕,",
                {len(fields) - 1: "<redacted-caption>"},
                set(range(len(fields))),
            )
            if fields is not None and len(fields) >= 2
            else "+TIRTC:字幕,<redacted-malformed>"
        )
    elif text.startswith("+TIRTC:绑定码,"):
        text = '+TIRTC:绑定码,"<redacted-binding-code>"'
    elif text.startswith("+TIRTC:设备,"):
        text = '+TIRTC:设备,"<redacted-device>"'
    elif text.startswith("+TIRTC:联系人,"):
        fields = parse_csv_urc(text, "+TIRTC:联系人,")
        text = (
            replace_csv_fields(
                text,
                "+TIRTC:联系人,",
            ...
            if fields is not None and len(fields) >= 4
            else "+TIRTC:联系人,<redacted-malformed>"
        )
    elif text.startswith("+TIRTC:好友申请,"):
        fields = parse_csv_urc(text, "+TIRTC:好友申请,")
        text = (
            replace_csv_fields(
                text,
                "+TIRTC:好友申请,",
            ...
            if fields is not None and len(fields) >= 2
            else "+TIRTC:好友申请,<redacted-malformed>"
        )
    ...
    def _reader_loop(self) -> None:
        ...
        while not self._stop.is_set():
            ...
            while True:
                ...
                try:
                    ...
                    with self._condition:
                        self._utf8_decode_errors += 1
                    self.evidence.record(
                        "utf8-error",
                        self.port,
                        (
                            f"invalid UTF-8 at byte {exc.start}; "
                            f"line_length={len(raw)}; "
                ...
                if "\ufffd" in text:
                    ...
                    self.evidence.record(
                        "utf8-error",
                        self.port,
                        "device emitted Unicode replacement character",
                        transport_epoch=self.transport_epoch,
                    )
                    continue
                if not text:
        ...
        if pending:
            try:
                ...
                invalid_utf8 = b"\xef\xbf\xbd" in tail
            ...
            with self._condition:
                if invalid_utf8:
                    ...
                    self._framing_errors += 1
            self.evidence.record(
                "utf8-error" if invalid_utf8 else "framing-error",
                self.port,
                (
                    "incomplete serial line at transport close; "
                    f"line_length={len(pending)}; "
                    f"sha256={hashlib.sha256(bytes(pending)).hexdigest()}"
                ),
                transport_epoch=self.transport_epoch,
            )
    ...
    def health_snapshot(self) -> dict[str, int | bool]:
        with self._condition:
            return {
                ...
                "utf8_decode_errors": self._utf8_decode_errors,
    ...
    def assert_healthy(
        ...
        *,
        ...
        unhealthy = (
            ...
            or health["utf8_decode_errors"] != 0
                    ...
                    for line in self._lines_after_locked(scanned):
                        ...
                        if FINAL_RE.match(line.text):
                            ...
                            expected_final = (
                                ...
                                else f"ERROR:{expected_error}"
                            )
                            if line.text != expected_final:
                                raise RuntimeError(
    ...
    def synchronize(self, timeout: float = 20.0) -> None:
        ...
        while time.monotonic() < deadline:
            try:
                ...
                self.send("AT+PROTO=RAW", timeout=2.0)
                return
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--expect-error", type=int)
...
def main() -> int:
    ...
    if args.expect_error is not None and args.action != "command":
        raise RuntimeError("--expect-error is only valid for the command action")
    ...
    try:
        ...
        if args.action == "probe":
            ...
            for device in devices:
                device.synchronize()
                device.send(args.command, expected_error=args.expect_error)
