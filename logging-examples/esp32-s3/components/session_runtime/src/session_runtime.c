...
static void media_prompt_callback(uint32_t generation,
...
static void handle_internal_event(session_internal_event_t *event)
{
    ...
    if (event_is_session_intent(event->type) &&
        ...
        if (s_context.owner == DEVICE_SERVICE_NONE &&
            ...
            s_context.ai_call_handoff.phase ==
                SESSION_RUNTIME_HANDOFF_NONE) {
            s_context.session_origin_request_id = 0U;
            s_context.last_origin_request_id = 0U;
        }
...
esp_err_t session_runtime_start(void)
{
    ...
    media_runtime_set_prompt_callback(media_prompt_callback, NULL);
    ...
    if (created != pdPASS) {
        media_runtime_set_prompt_callback(NULL, NULL);
...
{
    if (public_owner() != DEVICE_SERVICE_AI ||
        public_handoff_pending() || preset == NULL || preset[0] == '\0') {
        return ESP_ERR_INVALID_STATE;
    }
    return enqueue_intent(SESSION_INT_AI_PROMPT,
                          preset,
                          NULL,
                          false,
                          SESSION_RUNTIME_CALL_AUDIO,
