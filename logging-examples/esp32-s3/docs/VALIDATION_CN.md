# ESP32-S3 AT ThingConnect 日志示例：验证清单

精简中文模式用于普通体验；本清单涉及 schema、generation、request ID 和
原始状态快照的项目均在 `AT+PROTO=RAW` 结构化调试模式下执行。自动化工具
负责显式切换模式，验证完成后可用 `AT+PROTO=USER` 返回精简中文模式。

## 静态与构建

1. 使用 ESP-IDF `v5.5.4`，并用 `-B build` 固定本源码快照的唯一构建目录。
   目录名可替换，但不能同时维护多份正式输出，也不能复用其他 checkout 的
   构建目录；工程根
...
.\tools\validate_example.ps1 `
  -ReferenceLocalConfig <private-reference-config> `
  -FirmwarePath .\build\tirtc_esp32s3_at_thingconnect_demo.bin `
  -BuildConfigPath .\sdkconfig
```

`private-reference-config` 表示开发者本机、未纳入 Git 的参考配置路径；没有需要
做二进制比对的私有配置时可省略该参数。不传 `ReferenceLocalConfig` 与
`FirmwarePath` 时，脚本只执行 SDK/素材
...
python .\tools\at_scenarios.py suite --port-a $portA --port-b $portB `
  --build-dir .\build --media-seconds 8 --iterations 1 `
...
python .\tools\at_scenarios.py race-call-cancel `
  --port-a $portA --port-b $portB --build-dir .\build `
