# ESP32-S3 AT ThingConnect 日志示例：AT 指令
...
| 使用者 | 模式 | 入口 |
| --- | --- | --- |
| 人工体验 | 精简中文模式，启动后默认进入 | `AT+TIRTC=?` |
| 主机程序、脚本和深入联调 | 结构化调试模式 | `AT+PROTO=RAW` |

第一次使用建议先完成[根 README](../README.md) 的配网、绑定和 AI 体验，再
按本文接入结构化字段。两种模式控制同一个底层状态机，不需要重新烧录固件。

## 精简中文模式

设备每次启动默认进入面向开发者的精简中文模式：
...
`AT+TIRTC=?` 返回当前帮助；`AT+TIRTC?` 返回系统和会话摘要。配网、绑定、
AI、联系人和呼叫操作的完整中文命令见[使用指南](USER_GUIDE_CN.md)。同步
`OK` 只表示命令已受理，最终结果以 `+TIRTC:` 异步消息为准。

## 结构化调试模式

下面各节记录自动化回归和深入联调使用的结构化协议。发送
`AT+PROTO=RAW` 后进入该模式，发送 `AT+PROTO=USER` 可回到精简中文模式；
切换模式会清理尚未完成的表面层请求，不改变底层平台身份。
...
上行并请求视频下行。日志示例只统计和记录下行帧，不做显示解码。
