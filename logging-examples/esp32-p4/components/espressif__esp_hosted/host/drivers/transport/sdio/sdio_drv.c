...
#ifdef CONFIG_ESP_HOSTED_C6_C61_AUTO_COMPAT
		/*
		 * C6 and C61 boards share the P4 SDIO bus pins but route EN to
		 * different GPIOs. Only touch the alternate pin after passive probe
		 * and the configured reset path both failed, so a healthy board never
		 * sees activity on an unrelated GPIO.
		 */
		gpio_pin_t alternate_reset_pin = reset_pin;
		alternate_reset_pin.pin = reset_pin.pin == 54 ? 13 : 54;
		ESP_LOGW(TAG, "Trying alternate C6/C61 reset GPIO[%d]",
			 alternate_reset_pin.pin);
		transport_gpio_reset(bus_handle, alternate_reset_pin);
		res = transport_card_init(bus_handle, CARD_INIT_TIMEOUT_MS);
		if (!res) {
			ESP_LOGI(TAG, "Card init success with alternate reset GPIO");
			set_transport_state(TRANSPORT_RX_ACTIVE);
			return 0;
		}
		ESP_LOGE(TAG, "card init failed with both C6/C61 reset GPIOs");
#endif
	}
