...
ESP32-S3 Device Monitor `1.9.7` 的完整开发来源为 annotated Tag `v1.9.7`、Tag object
`73f6a3b18ac4ad5e95f813c6eba4f31907961be6`、commit
`58c2d152a179fa382511f4217fb84858072ffbb0`、tree
`206b2373a389fab39ece1893f23f1ddbf0e66dfe`；统一公开代码提交为
`a3c6d4c5bdecb215f8c8ebec554424de77ed193b`。正式构建输入与该提交的 repository tree
`7508d3f00fb436a0d9bfd716fc1fc152f2d0cf8a`、项目 tree
`ac13d559aae8f74e92afbf585fe79c5da02aa324` 完全一致。微信主动呼叫继续使用体验版
`wx_version_type=2`。

TiRTC SDK 以 `db7290f` 为功能基础，另含 `13e34c3` 的 HTTPS 服务端认证修复。公开静态库
为 `2,127,226` bytes，SHA-256 为
...
`1.9.7` 把呼叫动作与本地音量/增益操作移出 UI 回调，用 generation 阻止旧接听 worker
...
ESP32-S3 Device Monitor 旧版本的音频链路做过双板媒体包核对和 20,000 次 A-law 编解码
自检，但 `1.9.7` 尚未完成烧录、UI 动作队列、generation 防护、分阶段超时、新铃声或音量
触摸回归。当前 Web IPC 和设备
