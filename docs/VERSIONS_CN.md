...
| 项目 | 静态来源核验 | 正式构建 | 发布交付 |
| --- | --- | --- | --- |
| ESP32-P4 Device App `1.3.1` | Tag、版本、SDK 快照、源码增量、媒体参数和凭据已核对 | 最终公开快照执行 ESP-IDF `5.5.4` 干净构建，结果见本次 manifest | 源码与 `0x0` 完整镜像 |

P4 完整镜像 `esp32p4-tirtc-device-monitor-full-v1.3.1.bin` 只通过 Release 分发，不进入 Git；
大小为 `16,777,216` bytes，SHA-256 为
`3F55403C60CE371D81239CD5EC028FBFD1A27CAC0B693EB925CB1C60F3CFE1C5`，从 `0x0` 烧录。
本次不重复上传旧 S3 固件。目标板运行、通话长稳和故障恢复是独立证据层；尤其 TinyH264
helper 真正卡死后的任务回收仍需真机重复通话与长时间运行验证。
...
- 构建项目的 ESP-IDF、命令、产物用途、Flash 地址、大小和 SHA-256。
