...
| ESP32-P4 Device Monitor | `1.5.3` | [`esp32-p4-device-monitor-v1.5.3`](https://github.com/tangeai/tirtc-device-example/releases/tag/esp32-p4-device-monitor-v1.5.3) |
...
| P4 Device Monitor | `esp32p4-tirtc-device-monitor-full-v1.5.3.bin` | `16,777,216` | 见本 Release 的 `SHA256SUMS.txt` | ESP Tool，地址 `0x0` |
...
P4 Device Monitor `1.5.3` 提供 16 MiB 完整镜像、`SHA256SUMS.txt` 和
`release-manifest.json` 三个附件。已完成 ESP-IDF `5.5.4` 正式干净构建：应用镜像为
`6,973,376` bytes，APP 分区剩余 `563,264` bytes（`7.47%`）。完整镜像和清单均来自
本次构建，不复用旧版固件。设备呼叫上行为 `384x256@12fps`、目标
`256kbps` H264；微信设备上行为 `640x480@15fps`、目标 `800kbps` H264，下行请求
`640x480` MJPEG，实际帧可以更小。RTC 服务地址保持 HTTPS，不再按 SDK 版本降级为 HTTP。
下载时应从同一 `1.5.3` Release 获取完整附件，不混用旧版本源码、SDK 或固件。

## ESP32-S3/P4 完整镜像烧录

准备一根支持数据传输的 USB 线，并使用支持 Web Serial 的 Chrome 或 Edge。

1. 从目标项目 Release 下载文件名包含 `-full-` 的完整镜像、`SHA256SUMS.txt` 和
