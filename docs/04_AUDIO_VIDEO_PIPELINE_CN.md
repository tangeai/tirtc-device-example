...
![P4 视频从摄像头到 LCD 的处理链路](assets/developer-guide/chapter4/p4-video-pipeline-v3.png)

图中下方三档参数都指 **P4 设备端的 H.264 上行**，不是微信手机端采集参数，也不是服务端到设备的下行参数：

- IPC 上行：`1280×960@20fps`，目标 `4Mbps`。
- 设备通话上行：`384×256@12fps`，目标 `256kbps`。
- 微信通话上行：`640×480@15fps`，目标 `800kbps`。
- 微信通话下行：设备请求 `640×480` MJPEG，服务端实际帧可以更小；P4 硬件解码后以 `cover` 方式映射到 `480×320` 屏幕。当前工程没有为这条下行固定帧率或码率。

这些是 P4 Device Monitor `1.5.3` 的源码默认档位。目标码率不等于实测吞吐；SDK 反馈、画面运动量和编码器量化限制都会影响实际输出。
...
P4 当前还默认开启 SDK/TGMP 码率反馈：连接建立后注册码率范围，SDK 用 `on_update_bitrate()` 给出绝对目标码率。回调只投递控制事件，应用控制任务再调整编码器；旧的本地队列压力自动弱网调节保持关闭，避免两个控制器同时改码率。设备呼叫的调整范围为 `96–256kbps`，微信与 IPC 使用各自档位。

这说明控制链已经接入，实际效果仍需运行证据。应把 SDK 建议、应用接受值、编码器实际码率、队列年龄、丢包和 RTT 放在同一时间轴上，确认拥塞时下降、网络恢复后上升，并检查是否反复振荡。仅看到队列变浅或画面恢复，不能证明码率控制有效。
...
| P4 SDK 码率反馈 | SDK 目标、应用接受值、编码器实际码率、RTT、丢包和发送积压 | 拥塞时受控下降，干净窗口后逐步恢复，不与本地控制器争用 |
...
视频先记录静态画面基线，再让摄像头拍摄持续高运动内容，例如 30 fps 全屏视频。两组都要记录帧大小、编码耗时、实际码率、发送队列和内存水位。静态场景压缩率高，不能据此判断高运动场景是否还有余量。
...
弱网覆盖同一设备的入站与出站方向，并分别记录启用的延迟、抖动、丢包和带宽限制，避免只测到一个方向。测试结果还要标明具体固件版本；`1.5.3` 的正式构建和目标板证据见[版本与证据清单](VERSIONS_CN.md)，旧版本记录不自动延续到新版本。
