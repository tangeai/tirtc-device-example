...
static void emit_caption(session_context_t *context, const cJSON *params)
{
    ...
    bool has_seq = cJSON_IsNumber(seq_num) && seq_num->valuedouble >= 0;
    ...
    bool valid = (event.caption_type == 0 || event.caption_type == 1) &&
                 cJSON_IsString(text) && text->valuestring != NULL &&
                 caption_id_string(params,
                                   event.request_id,
...
static void finish_local_ai(session_context_t *context, const char *reason)
{
    int end_rc = 0;
    ...
    case SESSION_INT_AI_STOP:
        if (context->owner == DEVICE_SERVICE_AI) {
            if (handoff_reserved) {
                ...
                finish_local_ai(context, "ai-local-stop");
            }
        }
        break;
