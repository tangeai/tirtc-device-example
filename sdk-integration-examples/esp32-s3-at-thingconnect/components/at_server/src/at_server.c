...
#define AT_SERVER_URC_QUEUE_DEPTH 32
...
{
    return (TaskHandle_t)atomic_load_explicit(
        &s_task,
...
static esp_err_t write_locked_line(const char *line)
{
    ...
    int written = at_transport_write(&s_transport,
                                     line,
                                     length,
...
static esp_err_t queue_formatted_urc(const char *format, va_list arguments)
{
    ...
    if (xQueueSend(s_urc_queue, &line, 0) != pdTRUE) {
        free_urc_line(line);
        (void)atomic_fetch_add_explicit(&s_urc_dropped,
                                        1,
...
static void flush_pending_urcs(void)
{
    if (s_urc_queue == NULL) {
        return;
    }
    ...
    while (pending-- > 0U &&
           xQueueReceive(s_urc_queue, &line, 0) == pdTRUE) {
    ...
    if (dropped != 0U) {
        int length = snprintf(s_output_buffer,
                              sizeof(s_output_buffer),
                              "+AT:URC_OVERFLOW,%lu",
                              (unsigned long)dropped);
        if (length > 0 && (size_t)length < sizeof(s_output_buffer)) {
            (void)write_locked_line(s_output_buffer);
        }
        wipe_output_buffer();
    }
}
...
static esp_err_t parse_request(char *body,
                               at_server_request_t *request,
                               char *name,
                               size_t name_size)
{
    char *name_end = body;
...
static void at_server_task(void *argument)
{
    ...
    */
    ...
    while (atomic_load_explicit(&s_running, memory_order_acquire)) {
        const int received = at_transport_read(
            &s_transport,
            input,
            sizeof(input),
    ...
    discard_pending_urcs();
    wipe_output_buffer();
    if (atomic_exchange_explicit(&s_transport_owned,
                                 false,
                                 memory_order_acq_rel)) {
        (void)at_transport_drain(
            &s_transport,
            pdMS_TO_TICKS(AT_SERVER_WRITE_TIMEOUT_MS));
...
{
    if (config == NULL ||
        at_transport_validate(&config->transport) != ESP_OK ||
    ...
    for (size_t index = 0; index < config->command_count; ++index) {
        ...
        for (size_t duplicate = index + 1;
             duplicate < config->command_count;
             ++duplicate) {
    ...
    if (s_state_lock == NULL || s_started == NULL ||
        s_stopped == NULL || s_urc_queue == NULL) {
        return ESP_ERR_NO_MEM;
    }
...
esp_err_t at_server_start(void)
{
    ...
    atomic_store_explicit(
        &s_task,
        (uintptr_t)created_task,
        memory_order_release);
    if (xSemaphoreTake(s_started,
                       pdMS_TO_TICKS(AT_SERVER_START_TIMEOUT_MS)) != pdTRUE) {
        atomic_store_explicit(&s_running, false, memory_order_release);
        (void)xSemaphoreGive(s_state_lock);
        return ESP_ERR_TIMEOUT;
    }
...
esp_err_t at_server_stop(void)
{
    ...
    if (running_task != NULL &&
        xSemaphoreTake(
            s_stopped,
            pdMS_TO_TICKS(AT_SERVER_STOP_TIMEOUT_MS)) != pdTRUE) {
        (void)xSemaphoreGive(s_state_lock);
        return ESP_ERR_TIMEOUT;
    }
    wipe_output_buffer();
