...
本文面向希望通过串口 AT 指令体验 TiRTC 开发者平台、AI 对讲和设备呼叫的
使用者。完整 AT 字段定义见 [AT_COMMANDS_CN.md](AT_COMMANDS_CN.md)，本文只
保留从上电到完成一次体验所需的操作。
...
idf.py -B build-hardware-final -p <FLASH_PORT> flash
```

双机体验时，对 A、B 两块板分别执行烧录。`flash` 会写入 app 和当前
`media/` 生成的 `storage` SPIFFS 镜像。烧录完成后，串口终端连接对应的
`<AT_PORT_A>` 或 `<AT_PORT_B>`。

不要为普通升级执行 `erase-flash`。它会同时清除 Wi-Fi、设备身份和绑定凭据，
只有明确需要重新初始化设备时才使用。
...
没有扬声器 sink 是本日志例子的设计边界。下行音频被统计后丢弃，因此
“听不到声音”不代表网络媒体没有建立。
