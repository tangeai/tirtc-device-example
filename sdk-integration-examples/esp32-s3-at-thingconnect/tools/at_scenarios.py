    ...
    while time.monotonic() < deadline:
        ...
        audio_ok = (
            ...
            and (
                ...
                else last.rx_audio_frames > 0 and last.rx_audio_bytes > 0
            )
        )
        video_ok = not video or (
            ...
            and (
                ...
                if require_rx_growth
                else last.rx_video_frames > 0 and last.rx_video_bytes > 0
            )
        )
...
def wait_ai_caption(
    ...
    return board.at.wait_for(
        matches,
        timeout=timeout,
        description="non-empty AI caption",
        after=cursor,
        expected_epoch=transport_epoch,
    )
...
def wait_ai_call_action(
    ...
    if (
        ...
        or not target.strip()
        or target.strip().casefold() not in target_alias.casefold()
    ):
...
def scenario_ai_call_device(
    ...
    changed_remark = original.remark != target_alias

    try:
        if changed_remark:
            set_contact_remark(caller, callee.device_id, target_alias)
        ...
        for board in (caller, callee):
            ensure_idle(board)
        if changed_remark:
            set_contact_remark(caller, callee.device_id, original.remark)
...
def collect_build_identity(build_dir: Path) -> dict[str, object]:
    ...
    return {
        ...
        "media": {
            "audio": file_identity(
                REPO_ROOT
                / "media/audio_g711a_8khz_mono_20ms_10s_500packets.g711a"
            ),
            "video": file_identity(
                REPO_ROOT
                / "media/video_h264_annexb_640x480_15fps_10s_150frames.h264"
            ),
        },
        "metadata": {
...
def main() -> int:
    args = parse_args()
    if args.scenario not in ("preflight", "ai", "race-ai-stop") and not args.port_b:
