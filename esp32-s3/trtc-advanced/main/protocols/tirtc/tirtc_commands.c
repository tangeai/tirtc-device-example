...
#define TIRTC_SESSION_WEB_CONTROL_SEND_WAIT_TICKS  pdMS_TO_TICKS(80)
...
static bool tirtc_session_normalize_embedded_command(tirtc_session_event_t *event)
{
    if (event == NULL || event->payload.command.data == NULL ||
        event->payload.command.data_len < sizeof(uint32_t)) {
        return false;
    }

    const uint8_t *data = event->payload.command.data;
    uint32_t embedded_cmdw = tirtc_session_load_le32(data);
    uint16_t embedded_cmd = tirtc_session_get_command_id(embedded_cmdw);
    uint16_t current_cmd = tirtc_session_get_command_id(event->payload.command.cmdw);
    bool embedded_known = tirtc_session_is_showcase_command_id(embedded_cmd);
    bool current_known = tirtc_session_is_showcase_command_id(current_cmd);

    if (!embedded_known || (current_known && embedded_cmd != current_cmd)) {
        return false;
    }

    event->payload.command.cmdw = embedded_cmdw;
    event->payload.command.data = (uint8_t *)data + sizeof(uint32_t);
    event->payload.command.data_len -= sizeof(uint32_t);
    ESP_LOGD(TAG,
             "embedded command normalized: cmdw=0x%08lx payload_len=%lu",
             (unsigned long)embedded_cmdw,
...
static esp_err_t tirtc_session_send_web_control_response(tirtc_conn_t conn,
                                                        ...
                                                        size_t data_len)
{
    return tirtc_session_send_command_raw_wait(conn,
                                               ...
                                               data,
                                               data_len,
                                               TIRTC_SESSION_WEB_CONTROL_SEND_WAIT_TICKS);
}

static void tirtc_session_send_web_control_status(tirtc_conn_t conn,
                                                  uint16_t cmd,
                                                  const char *payload)
{
    if (payload == NULL || payload[0] == '\0') {
        ESP_LOGD(TAG, "web control status skipped: cmd=0x%04x empty payload", (unsigned)cmd);
        return;
    }

    size_t payload_len = strlen(payload);
    if (cmd == TIRTC_SESSION_CMD_VOLUME) {
        uint8_t volume = 0;
        if (!tirtc_session_parse_web_u8_assignment(payload, payload_len, 'v', 100U, &volume)) {
            ESP_LOGW(TAG,
                     "web volume status skipped: invalid payload=\"%s\"",
                     payload);
            return;
        }
    }

    uint32_t cmdw = tirtc_session_make_showcase_cmdw(cmd, (uint16_t)atomic_get_cmd_sn(), false);
    esp_err_t ret = tirtc_session_send_command_raw_wait(conn,
                                                        cmdw,
                                                        payload,
                                                        payload_len,
                                                        TIRTC_SESSION_WEB_CONTROL_SEND_WAIT_TICKS);
    if (ret != ESP_OK) {
        ESP_LOGD(TAG,
                 "web control status skipped: cmd=0x%04x ret=%s",
                 (unsigned)cmd,
                 esp_err_to_name(ret));
    }
}
...
void tirtc_session_handle_remote_command(const tirtc_session_event_t *event)
{
    ...
    tirtc_session_event_t normalized_event = *event;
    (void)tirtc_session_normalize_embedded_command(&normalized_event);
    ...
    case TIRTC_SESSION_CMD_VOLUME: {
        ...
        if (!tirtc_session_make_web_payload_view(event->payload.command.data,
            ...
            ESP_LOGW(TAG,
                     ...
                     (unsigned long)event->payload.command.cmdw,
                     (unsigned long)event->payload.command.data_len);
            (void)tirtc_session_send_web_control_response(event->payload.command.conn,
                                                         event->payload.command.cmdw,
                                                         "ERR",
                                                         strlen("ERR"));
        ...
        (void)tirtc_session_send_web_control_response(event->payload.command.conn,
                                                     ...
                                                     strlen(TIRTC_SESSION_MEDIA_TOGGLE_OK_TEXT));
        /* This path is the ACK for Web's v=N request. Active volume reports
         * must be ordinary 0x1102 commands with an explicit v=N payload.
         */
    ...
    case TIRTC_SESSION_CMD_DOOR: {
        ...
        (void)tirtc_session_send_web_control_response(event->payload.command.conn,
                                                     ...
                                                     TIRTC_SESSION_MEDIA_TOGGLE_OK_TEXT,
                                                     strlen(TIRTC_SESSION_MEDIA_TOGGLE_OK_TEXT));
        tirtc_session_send_web_control_status(event->payload.command.conn,
                                              TIRTC_SESSION_CMD_DOOR,
                                              door_open ? "d=1" : "d=0");
        ESP_LOGI(TAG,
