...
static esp_err_t app_call_contacts_save_current(void)
{
	...
	if (store.count == 0) {
		ret = platform_nvs_async_erase_key(APP_CALL_CONTACT_NVS_NAMESPACE, APP_CALL_CONTACT_NVS_KEY);
	} else {
		ret = platform_nvs_async_set_blob(APP_CALL_CONTACT_NVS_NAMESPACE,
						  APP_CALL_CONTACT_NVS_KEY,
						  &store,
						  sizeof(store));
	}

	if (ret == ESP_OK) {
		ESP_LOGD(TAG, "call contacts save queued: count=%u", (unsigned)store.count);
	} else {
		ESP_LOGW(TAG, "call contacts save queue failed: %s", esp_err_to_name(ret));
	}
	return ret;
}
