...
static const char s_index_html[] =
    ...
    "<title>ESP32-S3 Screen Debug</title>"
    "<style>"
    ...
    "<div class=\"top\"><h1>ESP32-S3 Screen Debug</h1><div id=\"status\" class=\"meta\">Connecting...</div></div>"
    ...
    "let paused=false,loading=false,w=320,h=240,lastTouch=null;"
    "function refresh(){if(paused||loading)return;loading=true;img.src='/screen.bmp?t='+Date.now();}"
    ...
    "statusEl.textContent=(s.ip||'no ip')+' '+w+'x'+h+' port '+s.port;}).catch(()=>{statusEl.textContent='offline'});}"
...
esp_err_t screen_debug_server_start(void)
{
    ...
    BaseType_t task_ret = xTaskCreate(screen_debug_task,
                                      "screen_debug",
