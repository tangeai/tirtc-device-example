...
static esp_err_t device_call_get_bearer(char *buffer, size_t buffer_size)
{
    ...
    token = heap_caps_calloc(1, sizeof(*token), MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    if (token == NULL) {
...
static esp_err_t device_call_post_room_action_async(device_call_action_t action,
                                                    ...
                                                    const char *reason)
{
    device_call_action_ctx_t *ctx = calloc(1, sizeof(*ctx));
    if (ctx == NULL) {
        return ESP_ERR_NO_MEM;
    }
    ctx->action = action;
...
static esp_err_t device_call_wait_for_disconnect(uint32_t timeout_ms)
{
    ...
    while (elapsed_ms < timeout_ms) {
        if (!device_call_transport_is_connected()) {
            return ESP_OK;
        }
        vTaskDelay(pdMS_TO_TICKS(DEVICE_CALL_POLL_INTERVAL_MS));
        elapsed_ms += DEVICE_CALL_POLL_INTERVAL_MS;
...
static esp_err_t device_call_send_connected_notice(const char *room_id)
{
    ...
    esp_err_t ret = rtc_transport_send_command(RTC_TRANSPORT_COMMAND_DEVICE_CALL_CONNECTED,
                                               payload,
                                               (size_t)written);
    ESP_LOGI(CALL_FLOW_TAG,
...
static void device_call_accept_task(void *arg)
{
    ...
    if (ctx->switch_from_active_call) {
        ...
        (void)rtc_transport_send_command(RTC_TRANSPORT_COMMAND_DEVICE_CALL_HANGUP, NULL, 0);
...
static esp_err_t device_call_query_current_room(device_call_room_info_t *info)
{
    ...
    /*
     * call-server uses `data,omitempty`; OK(nil) may therefore omit `data`
     * instead of serializing it as JSON null. Both forms mean no active room.
     */
...
static esp_err_t device_call_recover_stale_room(const char *reported_room_id)
{
    ...
    if (ret == ESP_OK) {
        ...
        // Keep the documented 40202 recovery path available if the room query is transiently unavailable.
        ESP_LOGW(TAG,
...
static void device_call_request_task(void *arg)
{
    ...
    if (device_call_generation_matches(ctx->generation)) {
        bool session_ended = false;
        device_call_request_ctx_t *timer_ctx = calloc(1, sizeof(*timer_ctx));
...
static void device_call_handle_incoming(const cJSON *payload, uint32_t message_generation)
{
    ...
    local_busy = s_call.pending_incoming ||
                 ...
                 (s_call.state != DEVICE_CALL_STATE_IDLE &&
                  s_call.state != DEVICE_CALL_STATE_ERROR);
    incoming_allowed = s_call.incoming_allowed;
    callback_ctx = s_call.callback_ctx;
    device_call_unlock();

    if (!local_busy && incoming_allowed != NULL) {
        bool heap_ok_before = heap_caps_check_integrity_all(false);
        ESP_LOGI(CALL_FLOW_TAG,
                 "stage=incoming_policy_begin gen=%lu room=%s peer=%s heap_ok=%d task=%s stack_hwm=%u",
        ...
        policy_allowed = incoming_allowed(callback_ctx);
        bool heap_ok_after = heap_caps_check_integrity_all(false);
        ESP_LOGI(CALL_FLOW_TAG,
                 "stage=incoming_policy_done gen=%lu room=%s peer=%s allowed=%d heap_ok=%d stack_hwm=%u",
                 (unsigned long)message_generation,
                 room_id,
                 caller_id,
                 policy_allowed ? 1 : 0,
    ...
    if (local_busy || !policy_allowed) {
        esp_err_t reject_ret = device_call_post_room_action_async(DEVICE_CALL_ACTION_REJECT,
                                                                  room_id,
                                                                  "busy");
        ESP_LOGW(CALL_FLOW_TAG,
                 "stage=incoming_busy_reject gen=%lu room=%s peer=%s local_busy=%d ret=%s",
                 (unsigned long)message_generation,
                 room_id,
                 caller_id,
                 local_busy ? 1 : 0,
    ...
    /* Revalidate after the application policy callback; another call may have
     * claimed the session while this MQTT message was being arbitrated. */
    device_call_lock();
    if (!s_call.ingress_enabled || s_call.generation != message_generation) {
        device_call_unlock();
        return;
    ...
    if (local_busy) {
        ...
        esp_err_t reject_ret = device_call_post_room_action_async(DEVICE_CALL_ACTION_REJECT,
                                                                  ...
                                                                  "busy");
        ESP_LOGW(CALL_FLOW_TAG,
                 "stage=incoming_race_busy_reject gen=%lu room=%s peer=%s ret=%s",
                 (unsigned long)message_generation,
                 room_id,
                 caller_id,
                 esp_err_to_name(reject_ret));
...
static void device_call_handle_room_cancel(const cJSON *payload, uint32_t message_generation)
{
    ...
    if (close_transport) {
        device_call_reset_caller_media_gate();
        /* MQTT callbacks must not block inside the RTC teardown path.  The
         * application callback queues resource release on the app control
         * task, which owns RTC and audio lifecycle transitions. */
        device_call_notify_session_ended();
    }
    ESP_LOGI(CALL_FLOW_TAG,
                                     ...
                                     void *ctx)
{
    ...
    json = heap_caps_malloc(payload_len + 1U, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    if (json == NULL) {
        ESP_LOGW(CALL_FLOW_TAG,
...
static bool device_call_on_rtc_command(tirtc_conn_t conn,
                                       ...
                                       void *ctx)
{
    uint16_t command = (uint16_t)(cmdw & ~RTC_TRANSPORT_COMMAND_RESPONSE_BIT);
    char room_id[96] = {0};
    bool matched = false;
    bool expected = false;
    ...
    if (command == RTC_TRANSPORT_COMMAND_DEVICE_CALL_HANGUP) {
        ...
        ESP_LOGI(CALL_FLOW_TAG,
                 ...
                 RTC_TRANSPORT_COMMAND_DEVICE_CALL_HANGUP,
                 active ? 1 : 0);
        return false;
    }
    if (command != RTC_TRANSPORT_COMMAND_DEVICE_CALL_CONNECTED) {
        return false;
    }
...
esp_err_t device_call_init(const device_call_config_t *config)
{
    ...
    if (s_call.lock == NULL) {
        s_call.lock = xSemaphoreCreateMutex();
        if (s_call.lock == NULL) {
            return ESP_ERR_NO_MEM;
        }
    }
    ...
    s_call.incoming_allowed = config->incoming_allowed;
...
{
    device_call_accept_ctx_t *ctx = calloc(1, sizeof(*ctx));
    uint32_t generation = 0;
    device_call_state_t rejected_state = DEVICE_CALL_STATE_IDLE;
...
esp_err_t device_call_hangup(void)
{
    ...
    ESP_LOGI(CALL_FLOW_TAG,
             ...
             connected ? 1 : 0);
    if (connected) {
        esp_err_t command_ret = rtc_transport_send_command(RTC_TRANSPORT_COMMAND_DEVICE_CALL_HANGUP,
                                                           NULL,
                                                           0);
        ESP_LOGI(CALL_FLOW_TAG,
