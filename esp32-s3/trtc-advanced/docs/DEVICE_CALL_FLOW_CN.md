...
3. ready 后设置“下一条入站连接先接收、暂缓媒体”，再调用 `POST /v1/call/request`。
...
9. 主叫校验 `room_id`，匹配后进入 `IN_CALL` 并启动双向音频；不匹配则断开连接。
...
6. P2P 连接成功后发送 `0x2000` 房间确认，进入 `IN_CALL`，启动麦克风采集、上行发送和扬声器播放。
...
stage=call_request_done ... ret=ESP_OK
stage=ringing ...
