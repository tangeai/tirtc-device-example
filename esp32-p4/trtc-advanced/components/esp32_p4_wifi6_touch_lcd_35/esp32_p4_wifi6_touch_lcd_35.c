...
static bool s_touch_filter_valid = false;
static uint16_t s_touch_filter_x = 0;
static uint16_t s_touch_filter_y = 0;
static int64_t s_touch_filter_last_us = 0;
uint8_t brightness;
...
static uint16_t bsp_touch_clamp(uint16_t value, uint16_t max_exclusive)
{
    if (max_exclusive == 0) {
        return 0;
    }
    return value < max_exclusive ? value : (uint16_t)(max_exclusive - 1U);
}

#define BSP_TOUCH_RELEASE_GAP_US 30000LL
#define BSP_TOUCH_DEADBAND_PX    3U
#define BSP_TOUCH_FAST_MOVE_PX   24U

static uint16_t bsp_touch_filter_axis(uint16_t stable, uint16_t current)
{
    int32_t delta = (int32_t)current - (int32_t)stable;
    uint16_t magnitude = (uint16_t)(delta < 0 ? -delta : delta);

    if (magnitude <= BSP_TOUCH_DEADBAND_PX) {
        return stable;
    }
    if (magnitude >= BSP_TOUCH_FAST_MOVE_PX) {
        return current;
    }

    return (uint16_t)((int32_t)stable + (delta / 2));
}

static void bsp_touch_process_coordinates(esp_lcd_touch_handle_t tp,
                                          uint16_t *x,
                                          uint16_t *y,
                                          uint16_t *strength,
                                          uint8_t *point_num,
                                          uint8_t max_point_num)
{
    ...
    if (count == 0) {
        s_touch_filter_valid = false;
        s_touch_filter_last_us = now_us;
        return;
    }

    bool new_press = !s_touch_filter_valid ||
                     (s_touch_filter_last_us > 0 &&
                      now_us - s_touch_filter_last_us > BSP_TOUCH_RELEASE_GAP_US);
    for (uint8_t i = 0; i < count; ++i) {
        /*
         * Keep touch in the panel's physical 320x480 coordinate space here.
         * LVGL v8 rotates pointer coordinates again when the display is set
         * to LV_DISP_ROT_270, so pre-rotating here would double-transform
         * normal UI touches.
         */
        const uint16_t raw_x = bsp_touch_clamp(x[i], BSP_LCD_H_RES);
        const uint16_t raw_y = bsp_touch_clamp(y[i], BSP_LCD_V_RES);

        if (i == 0) {
            if (new_press) {
                s_touch_filter_x = raw_x;
                s_touch_filter_y = raw_y;
            } else {
                s_touch_filter_x = bsp_touch_filter_axis(s_touch_filter_x, raw_x);
                s_touch_filter_y = bsp_touch_filter_axis(s_touch_filter_y, raw_y);
            }
            x[i] = s_touch_filter_x;
            y[i] = s_touch_filter_y;
        } else {
            x[i] = raw_x;
            y[i] = raw_y;
        }
    }
    s_touch_filter_valid = true;
    s_touch_filter_last_us = now_us;
}
