...
static int ssl_check_server_ecdh_params( const mbedtls_ssl_context *ssl )
{
    ...
    mbedtls_printf( "[tls] ecdh stage=server-params curve=%s\n",
                    curve_info->name );
                                         ...
                                         unsigned char *end )
{
                                  ...
                                  (const unsigned char **) p, end ) ) != 0 )
    {
        ...
        mbedtls_printf( "[tls] ecdh stage=read-server-params failed "
                        "ret=%d\n", ret );
#if defined(MBEDTLS_SSL_ECP_RESTARTABLE_ENABLED)
...
static int ssl_write_client_key_exchange( mbedtls_ssl_context *ssl )
{
        ...
        ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_ECDH_ECDSA )
    {
        ...
        if( ret != 0 )
        {
            ...
            mbedtls_printf( "[tls] ecdh stage=make-public failed "
                            "ret=%d\n", ret );
#if defined(MBEDTLS_SSL_ECP_RESTARTABLE_ENABLED)
            if( ret == MBEDTLS_ERR_ECP_IN_PROGRESS )
                ret = MBEDTLS_ERR_SSL_CRYPTO_IN_PROGRESS;
        ...
        mbedtls_printf( "[tls] ecdh stage=make-public ok bytes=%u\n",
...
ecdh_calc_secret:
                                  ...
                                  ssl->conf->f_rng, ssl->conf->p_rng ) ) != 0 )
        {
            ...
            mbedtls_printf( "[tls] ecdh stage=calc-secret failed "
                            "ret=%d\n", ret );
#if defined(MBEDTLS_SSL_ECP_RESTARTABLE_ENABLED)
            if( ret == MBEDTLS_ERR_ECP_IN_PROGRESS )
                ret = MBEDTLS_ERR_SSL_CRYPTO_IN_PROGRESS;
        ...
        mbedtls_printf( "[tls] ecdh stage=calc-secret ok bytes=%u\n",
                        (unsigned int) ssl->handshake->pmslen );
        MBEDTLS_SSL_DEBUG_ECDH( 3, &ssl->handshake->ecdh_ctx,
