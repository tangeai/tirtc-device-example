...
void *hosted_thread_create(char *tname, uint32_t tprio, uint32_t tstack_size, void (*start_routine)(void const *), void *sr_arg)
{
	...
	uint32_t stack_caps = MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT;
...
#endif
	task_created = xTaskCreateWithCaps((void (*)(void *))start_routine,
			tname,
			tstack_size,
			sr_arg,
			tprio,
			thread_handle,
			stack_caps);
	if (task_created != pdTRUE || !(*thread_handle)) {
		*thread_handle = NULL;
		task_created = xTaskCreate((void (*)(void *))start_routine,
				tname,
				tstack_size,
				sr_arg,
				tprio,
