...
static esp_err_t transport_drv_no_tx_buffer(uint8_t if_type, volatile uint32_t *drop_count)
{
	uint32_t count = 0;
	...
	if (count == 1 || ((count % 32) == 0)) {
		...
		ESP_LOGW(TAG,
			 "Drop ESP-Hosted TX packet: if=%u no transport buffer count=%" PRIu32 " dma_free=%u dma_largest=%u",
			 if_type,
			 count,
			 (unsigned)dma_free,
...
static esp_err_t transport_drv_sta_tx(void *h, void *buffer, size_t len)
{
	...
	if (!copy_buff) {
		...
		pkt_stats.sta_tx_in_drop++;
	#endif
		return transport_drv_no_tx_buffer(ESP_STA_IF, &s_sta_tx_alloc_drop_count);
	}
...
static esp_err_t transport_drv_ap_tx(void *h, void *buffer, size_t len)
{
	...
	if (!copy_buff) {
		return transport_drv_no_tx_buffer(ESP_AP_IF, &s_ap_tx_alloc_drop_count);
	}
	g_h.funcs->_h_memcpy(copy_buff+H_ESP_PAYLOAD_HEADER_OFFSET, buffer, len);
