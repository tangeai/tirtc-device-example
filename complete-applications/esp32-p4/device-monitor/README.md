...
`1.5.3` 已完成 ESP-IDF `5.5.4` 正式干净构建。应用镜像为 `6,973,376` bytes，APP 分区
剩余 `563,264` bytes（`7.47%`）；16 MiB 完整镜像和 SHA-256 清单见本项目 Release。
本版本供开发者在目标板继续集成验证；烧录、HTTPS 正反例、绑定、微信正式版实呼和媒体回归
尚未完成。
