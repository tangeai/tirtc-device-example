...
#define DISPLAY_CALL_QR_SIZE 160
#define DISPLAY_CALL_QR_FULLSCREEN_SIZE 192
#define DISPLAY_CALL_QR_FULLSCREEN_TOP 8
#define DISPLAY_CALL_QR_FULLSCREEN_LABEL_TOP 208
#define DISPLAY_CALL_TERMINAL_HOLD_US (2500LL * 1000LL)
...
typedef enum {
    ...
    DISPLAY_WIFI_CONNECT_STATE_AUTH_FAILED,
    DISPLAY_WIFI_CONNECT_STATE_AP_NOT_FOUND,
...
static display_page_id_t s_wifi_parent_page = DISPLAY_PAGE_HOME;
static display_page_id_t s_uuid_parent_page = DISPLAY_PAGE_HOME;
...
static uint8_t s_device_receive_restore_volume = 80U;
...
static lv_obj_t *s_wifi_connection_runtime_label;
...
static lv_obj_t *s_wifi_connect_progress_label;
...
static lv_obj_t *s_call_status_card;
static lv_obj_t *s_call_peer_name_label;
static lv_obj_t *s_call_peer_id_label;
static lv_obj_t *s_call_state_label;
static lv_obj_t *s_call_state_detail_label;
static lv_obj_t *s_call_duration_row;
static lv_obj_t *s_call_duration_label;
static lv_obj_t *s_call_mic_row;
static lv_obj_t *s_call_mic_value_label;
static lv_obj_t *s_call_speaker_row;
static lv_obj_t *s_call_speaker_value_label;
static lv_obj_t *s_call_hangup_btn;
static lv_obj_t *s_call_qrcode;
static lv_obj_t *s_call_device_id_label;
static lv_obj_t *s_call_qr_overlay;
static lv_obj_t *s_call_qr_overlay_qrcode;
static lv_obj_t *s_call_qr_overlay_device_id_label;
...
static lv_obj_t *s_call_list_content;
...
static EXT_RAM_BSS_ATTR char s_call_active_peer_id[DISPLAY_CALL_CONTACT_DEVICE_ID_MAX];
static EXT_RAM_BSS_ATTR char s_call_terminal_message[DISPLAY_CALL_STATUS_MESSAGE_MAX];
static int64_t s_call_active_started_us;
static int64_t s_call_terminal_visible_until_us;
static display_call_state_t s_call_page_entry_state;
static display_call_role_t s_call_page_entry_role;
...
static void display_call_contact_touch_cb(lv_event_t *event);
...
static void display_tirtc_config_scan_btn_cb(lv_event_t *event);
...
static const lv_font_t *display_runtime_utf8_font(void);
...
#define DISPLAY_DEVICE_SPEAKER_VOLUME_RESTORE_DEFAULT 80U
#define DISPLAY_DEVICE_CAPTURE_GAIN_RESTORE_DEFAULT 50U
...
#define DISPLAY_CALL_CONTACT_TOUCH_LEFT      220
#define DISPLAY_CALL_CONTACT_TOUCH_WIDTH     84
#define DISPLAY_CALL_CONTACT_TOUCH_HEIGHT    44
...
#define DISPLAY_TIRTC_VERSION_TEXT           "TiRTC 2.3.0"
...
typedef struct {
    ...
    char pair_key[DISPLAY_TIRTC_CONFIG_TEXT_MAX];
...
static display_contact_scan_result_event_t s_wechat_scan_fallback_event;
static bool s_wechat_scan_fallback_busy;
...
static EXT_RAM_BSS_ATTR display_call_pending_contact_t s_call_pending_contacts[DISPLAY_CALL_CONTACT_COUNT];
static lv_obj_t *s_call_contact_call_buttons[DISPLAY_CALL_CONTACT_COUNT];
static bool s_call_contact_long_pressed[DISPLAY_CALL_CONTACT_COUNT];
                                             ...
                                             const display_status_t *status)
{
    ...
    if (device_id == NULL ||
        strlen(device_id) != DISPLAY_CALL_CONTACT_DEVICE_ID_LENGTH ||
        strlen(device_id) >= payload_size) {
        payload[0] = '\0';
        return false;
    }
...
static void display_invalidate_call_list_page(void)
{
    if (s_call_list_page != NULL) {
        ...
        s_call_list_content = NULL;
    }
    memset(s_call_contact_call_buttons, 0, sizeof(s_call_contact_call_buttons));
    memset(s_call_contact_long_pressed, 0, sizeof(s_call_contact_long_pressed));
}
...
static void display_call_scan_result_async_cb(void *arg)
{
    ...
    if (event->result == ESP_OK) {
        ...
        if (ret != ESP_OK) {
            ...
            display_show_wifi_alert("Add Contact", "Contact request failed.");
            free(event);
            return;
        }
        ...
        } else if (event->result == ESP_ERR_NOT_FOUND) {
            display_show_wifi_alert("扫码添加", "联系人不存在");
        } else {
...
static void display_release_wechat_scan_result_event(display_contact_scan_result_event_t *event)
{
    if (event == &s_wechat_scan_fallback_event) {
        memset(&s_wechat_scan_fallback_event, 0, sizeof(s_wechat_scan_fallback_event));
        s_wechat_scan_fallback_busy = false;
        return;
    }
    free(event);
}
...
static void display_wechat_scan_result_async_cb(void *arg)
{
    ...
    if (event->result == ESP_OK) {
        ...
        display_show_wifi_alert("扫码添加", "该联系人已在授权列表");
    } else {
        display_show_wechat_add_page();
        if (event->result == ESP_ERR_INVALID_RESPONSE && event->raw_payload[0] != '\0') {
            display_show_wifi_alert("QR Payload", event->raw_payload);
        } else if (event->result == ESP_ERR_INVALID_STATE) {
            display_show_wifi_alert("扫码添加", "授权列表同步中，请稍后重试");
        } else if (event->result == ESP_ERR_NOT_FOUND) {
            display_show_wifi_alert("扫码添加", "该用户尚未授权此设备");
        } else if (event->result == ESP_ERR_NO_MEM) {
            display_show_wifi_alert("扫码添加", "内存不足");
        } else {
            display_show_wifi_alert("扫码添加", display_contact_scan_error_text(event->result));
        }
    }

    display_release_wechat_scan_result_event(event);
}

static bool display_queue_wechat_scan_result(display_contact_scan_result_event_t *event,
                                             bool log_failure)
{
    if (event == NULL) {
        return true;
    ...
    if (!lvgl_port_lock(1)) {
        if (log_failure) {
            ESP_LOGW(TAG,
                     "wechat scan result dispatch waits for lvgl lock: result=%s",
                     esp_err_to_name(event->result));
        }
        return false;
    }
    ...
    if (async_ret != LV_RES_OK) {
        if (log_failure) {
            ESP_LOGW(TAG,
                     "wechat scan result dispatch failed: async result=%d",
                     (int)async_ret);
        }
        return false;
    }
...
static void display_wechat_scan_result_dispatch_task(void *arg)
{
    display_contact_scan_result_event_t *event = (display_contact_scan_result_event_t *)arg;

    for (uint8_t attempt = 0U; attempt < DISPLAY_SCAN_RESULT_DISPATCH_RETRY_COUNT; ++attempt) {
        if (display_queue_wechat_scan_result(event, attempt == 0U)) {
            platform_task_reaper_delete_current_with_caps(TAG);
            return;
        }
        vTaskDelay(pdMS_TO_TICKS(DISPLAY_SCAN_RESULT_DISPATCH_RETRY_MS));
    ...
    ESP_LOGW(TAG, "wechat scan result dispatch abandoned after retries");
    s_call_scan_active = false;
    display_release_wechat_scan_result_event(event);
    platform_task_reaper_delete_current_with_caps(TAG);
}
                                          ...
                                          void *ctx)
{
    ...
    if (event == NULL) {
        if (!s_wechat_scan_fallback_busy) {
            s_wechat_scan_fallback_busy = true;
            memset(&s_wechat_scan_fallback_event, 0, sizeof(s_wechat_scan_fallback_event));
            event = &s_wechat_scan_fallback_event;
            ESP_LOGW(TAG, "wechat scan result uses static fallback event");
        } else {
            s_call_scan_active = false;
            ESP_LOGW(TAG, "wechat scan result event unavailable");
            return;
        }
    }
    ...
    if (!display_queue_wechat_scan_result(event, false)) {
        BaseType_t task_ret = xTaskCreatePinnedToCoreWithCaps(
            display_wechat_scan_result_dispatch_task,
            "wechat_ui_evt",
            DISPLAY_SCAN_RESULT_DISPATCH_TASK_STACK,
            event,
            DISPLAY_SCAN_RESULT_DISPATCH_TASK_PRIORITY,
            ...
            DISPLAY_SCAN_RESULT_DISPATCH_TASK_ALLOC_CAPS);
        if (task_ret != pdPASS) {
            ESP_LOGW(TAG, "wechat scan result dispatch task create failed");
            for (uint8_t attempt = 0U;
                 attempt < DISPLAY_SCAN_RESULT_DISPATCH_RETRY_COUNT;
                 ++attempt) {
                if (display_queue_wechat_scan_result(event, attempt == 0U)) {
                    return;
                }
...
static void display_call_alert_accept_btn_cb(lv_event_t *event)
{
    ...
    if (s_actions.on_accept_call != NULL) {
        ESP_LOGI(CALL_FLOW_TAG, "stage=ui_accept_tap");
        ...
        if (ret != ESP_OK) {
            ...
            s_call_terminal_visible_until_us = 0;
            s_call_page_entry_state = DISPLAY_CALL_STATE_CONNECTING;
            s_call_page_entry_role = DISPLAY_CALL_ROLE_CALLEE;
            if (s_last_status.call_peer_device_id[0] != '\0') {
                strlcpy(s_call_active_peer_id,
                        s_last_status.call_peer_device_id,
                        sizeof(s_call_active_peer_id));
            }
...
static void display_call_alert_reject_btn_cb(lv_event_t *event)
{
    ...
    if (s_actions.on_reject_call != NULL) {
        ESP_LOGI(CALL_FLOW_TAG, "stage=ui_reject_tap");
...
static void display_show_call_alert(bool wechat)
{
    ...
    /* Ringing is an overlay, not a media-owner transition. Keep the current
     * foreground session alive until the user accepts; the accept callback
     * enters the matching app and performs the RTC/audio handoff. */
...
static void display_show_call_contact_request(const char *device_id)
{
    ...
    lv_obj_set_size(s_call_contact_request_box, 320, 240);
    lv_obj_set_style_bg_color(s_call_contact_request_box, lv_color_hex(0x10233B), 0);
    lv_obj_set_style_bg_opa(s_call_contact_request_box, LV_OPA_20, 0);
    lv_obj_set_style_border_width(s_call_contact_request_box, 0, 0);
    lv_obj_set_style_pad_all(s_call_contact_request_box, 0, 0);
    lv_obj_clear_flag(s_call_contact_request_box, LV_OBJ_FLAG_SCROLLABLE);
    lv_obj_move_foreground(s_call_contact_request_box);

    card = display_create_figma_box(s_call_contact_request_box,
                                    44,
                                    58,
                                    232,
                                    124,
...
static void display_update_call_contact_request(const display_status_t *status)
{
    ...
    if (s_call_contact_request_suppressed_peer[0] != '\0') {
        ...
        if (still_pending && status->call_contacts_last_error != ESP_OK) {
            display_show_wifi_alert("添加联系人", "添加失败");
            return;
        }
    }
...
static void display_show_call_delete_confirm(uint8_t contact_index)
{
    ...
    if (!s_call_contacts[contact_index].deletable) {
        ESP_LOGI(CALL_FLOW_TAG,
                 "stage=contact_delete_blocked peer=%s reason=same_account_auto_contact",
                 s_call_contacts[contact_index].device_id);
        display_show_wifi_alert("删除联系人", "Same-account contact\nReset device on Web");
        return;
    }
    ...
    (void)display_create_figma_text(card,
                                    ...
                                    12,
                                    LV_TEXT_ALIGN_CENTER);
    (void)display_create_figma_button(card,
                                      18,
                                      78,
                                      88,
                                      32,
...
static void display_update_wifi_connect_status_line(const display_status_t *status)
{
    ...
    if (s_selected_ssid[0] == '\0') {
        ...
        case DISPLAY_WIFI_CONNECT_STATE_CONNECTING: {
            ...
            use_runtime_hint = true;
            break;
        }
        case DISPLAY_WIFI_CONNECT_STATE_FAILED:
            strlcpy(hint_text, "连接失败", sizeof(hint_text));
            hint_color = lv_color_hex(0xE45656);
            use_runtime_hint = true;
            break;
        case DISPLAY_WIFI_CONNECT_STATE_AUTH_FAILED:
            strlcpy(hint_text, "密码错误", sizeof(hint_text));
            hint_color = lv_color_hex(0xE45656);
            break;
        case DISPLAY_WIFI_CONNECT_STATE_AP_NOT_FOUND:
            strlcpy(hint_text, "未找到该热点", sizeof(hint_text));
            hint_color = lv_color_hex(0xE45656);
            use_runtime_hint = true;
            break;
    ...
    if (use_runtime_hint && s_wifi_connect_progress_label != NULL) {
        lv_obj_add_flag(s_wifi_connect_hint_label, LV_OBJ_FLAG_HIDDEN);
        display_text_set_color(s_wifi_connect_progress_label, hint_color, 0);
        display_text_set(s_wifi_connect_progress_label, hint_text);
        lv_obj_clear_flag(s_wifi_connect_progress_label, LV_OBJ_FLAG_HIDDEN);
    } else {
        if (s_wifi_connect_progress_label != NULL) {
            lv_obj_add_flag(s_wifi_connect_progress_label, LV_OBJ_FLAG_HIDDEN);
        }
...
static void display_set_wifi_connect_state(display_wifi_connect_state_t state)
{
    s_wifi_connect_state = state;
    display_update_wifi_connect_status_line(&s_last_status);
}
...
static void display_submit_wifi_connect(void)
{
    ...
    if (s_selected_ssid[0] == '\0') {
        display_set_wifi_connect_state(DISPLAY_WIFI_CONNECT_STATE_SELECT_FIRST);
        return;
    }
    ...
    if (s_actions.on_wifi_connect == NULL) {
        display_set_wifi_connect_state(DISPLAY_WIFI_CONNECT_STATE_UNAVAILABLE);
        return;
    }
    if (requires_password && !using_saved_password && password_len < DISPLAY_WIFI_PASSWORD_MIN_LEN) {
        display_set_wifi_connect_state(DISPLAY_WIFI_CONNECT_STATE_SHORT_PASSWORD);
    ...
    if (ret == ESP_OK) {
        ...
        display_set_wifi_connect_state(DISPLAY_WIFI_CONNECT_STATE_CONNECTING);
        display_set_password_border_color(lv_color_hex(0xC89F4A));
        display_set_main_hint("Ready");
    } else {
        s_wifi_connect_pending = false;
        s_wifi_connect_target_ssid[0] = '\0';
        display_set_wifi_connect_state(DISPLAY_WIFI_CONNECT_STATE_FAILED);
        display_set_password_border_color(lv_color_hex(0xC8513C));
    }
}
...
static void display_wechat_add_edit_textarea_event_cb(lv_event_t *event)
{
    ...
    if (code == LV_EVENT_FOCUSED && s_wechat_add_edit_keyboard != NULL) {
        display_show_text_keyboard(s_wechat_add_edit_keyboard, target);
    }
    if (code == LV_EVENT_VALUE_CHANGED) {
...
static const lv_font_t *display_ai_chat_font(void)
{
    return display_runtime_utf8_font();
}
                                           ...
                                           lv_text_align_t align)
{
    ...
    if (ui_text_asset_has_cjk(text)) {
        ...
        if (img != NULL) {
            ...
            ESP_LOGW(TAG, "missing figma text asset: text=\"%s\" size=%u",
                     text != NULL ? text : "", (unsigned)font_size);
            return img;
        }
    }
...
static void display_text_set_uint(lv_obj_t *obj, uint32_t value)
{
    char text[11];

    (void)snprintf(text, sizeof(text), "%lu", (unsigned long)value);
    display_text_set(obj, text);
}
...
static void display_call_remark_textarea_event_cb(lv_event_t *event)
{
    ...
    if (code == LV_EVENT_VALUE_CHANGED) {
        ...
        if (s_call_remark_length_label != NULL) {
...
static bool display_ai_chat_should_show_new_chat_button(const display_status_t *status)
{
    if (status == NULL ||
        ...
        status->ai_chat_output_playback_pending ||
        s_actions.on_start_ai_chat == NULL) {
        return false;
    }
...
{
    lv_obj_t *header = display_create_figma_header_base(page, title, back_cb);
...
static void display_create_call_list_refresh_button(lv_obj_t *header)
{
    lv_obj_t *button = lv_btn_create(header);
    lv_obj_t *image = NULL;

    lv_obj_remove_style_all(button);
    lv_obj_set_pos(button, 254, 0);
    lv_obj_set_size(button, 58, 28);
    lv_obj_set_style_bg_opa(button, LV_OPA_TRANSP, 0);
    lv_obj_set_style_pad_all(button, 0, 0);
    lv_obj_clear_flag(button, LV_OBJ_FLAG_SCROLLABLE);
    lv_obj_add_event_cb(button, display_call_list_refresh_btn_cb, LV_EVENT_ALL, NULL);

    image = lv_img_create(button);
    lv_img_set_src(image, &call_contacts_refresh_default_img);
    lv_obj_set_pos(image, 8, 3);
                                                  ...
                                                  display_ai_setting_action_t action)
{
    ...
    /* Volume changes should react on touch-down.  Waiting for CLICKED adds the
     * whole finger hold/release time without improving volume safety. */
                                                             ...
                                                             const display_ai_chat_message_t *latest_message)
{
    ...
    if (status->ai_chat_cloud_speaking || status->ai_chat_output_playback_pending) {
                                                              ...
                                                              lv_obj_t **text_label)
{
    ...
    lv_obj_add_event_cb(btn,
                        ...
                        mute ? LV_EVENT_CLICKED : LV_EVENT_PRESSED,
                        (void *)(uintptr_t)action);
                                                       ...
                                                       lv_obj_t **mute_label)
{
    ...
    (void)display_create_device_volume_button(card, 4, "-10", false, down_action, NULL);
    (void)display_create_device_volume_button(card, 50, "+10", false, up_action, NULL);
    (void)display_create_device_volume_button(card, 96, "禁音", true, mute_action, mute_label);
}
...
static void display_create_call_qr_overlay(lv_obj_t *parent);

static void display_hide_call_qr_overlay(void)
{
    if (s_call_qr_overlay != NULL) {
        lv_obj_add_flag(s_call_qr_overlay, LV_OBJ_FLAG_HIDDEN);
    }
}

static void display_show_call_qr_overlay(void)
{
    if (s_call_qr_overlay == NULL) {
        display_create_call_qr_overlay(s_call_page);
    }
    if (s_call_qr_overlay == NULL) {
        return;
    }
...
#if LV_USE_QRCODE
    if (s_call_qr_overlay_qrcode != NULL && s_call_qr_payload[0] != '\0' &&
        !display_update_device_qr_object(s_call_qr_overlay_qrcode, s_call_qr_payload)) {
        ESP_LOGW(TAG, "call qr fullscreen update failed");
    }
#endif
    if (s_call_qr_overlay_device_id_label != NULL) {
        display_text_set(s_call_qr_overlay_device_id_label, s_call_qr_payload);
    }

    lv_obj_clear_flag(s_call_qr_overlay, LV_OBJ_FLAG_HIDDEN);
    lv_obj_move_foreground(s_call_qr_overlay);
}

static void display_call_qr_tap_cb(lv_event_t *event)
{
    if (lv_event_get_code(event) != LV_EVENT_CLICKED) {
        return;
    }

    if (s_call_qr_overlay == NULL || lv_obj_has_flag(s_call_qr_overlay, LV_OBJ_FLAG_HIDDEN)) {
        display_show_call_qr_overlay();
    } else {
        display_hide_call_qr_overlay();
    }
}
...
static void display_create_call_qr_overlay(lv_obj_t *parent)
{
    ...
    s_call_qr_overlay = lv_obj_create(parent);
    lv_obj_remove_style_all(s_call_qr_overlay);
    lv_obj_set_pos(s_call_qr_overlay, 0, 0);
    lv_obj_set_size(s_call_qr_overlay, DISPLAY_DRIVER_WIDTH, DISPLAY_DRIVER_HEIGHT);
    lv_obj_set_style_bg_color(s_call_qr_overlay, lv_color_hex(0xFFFFFF), 0);
    lv_obj_set_style_bg_opa(s_call_qr_overlay, LV_OPA_COVER, 0);
    lv_obj_set_style_border_width(s_call_qr_overlay, 0, 0);
    lv_obj_set_style_pad_all(s_call_qr_overlay, 0, 0);
    lv_obj_clear_flag(s_call_qr_overlay, LV_OBJ_FLAG_SCROLLABLE);
    lv_obj_add_flag(s_call_qr_overlay, LV_OBJ_FLAG_CLICKABLE | LV_OBJ_FLAG_HIDDEN);
    lv_obj_add_event_cb(s_call_qr_overlay, display_call_qr_tap_cb, LV_EVENT_CLICKED, NULL);

#if LV_USE_QRCODE
    s_call_qr_overlay_qrcode = lv_qrcode_create(s_call_qr_overlay,
                                                DISPLAY_CALL_QR_FULLSCREEN_SIZE,
                                                lv_color_hex(0x111111),
                                                lv_color_hex(0xFFFFFF));
    lv_obj_align(s_call_qr_overlay_qrcode,
                 LV_ALIGN_TOP_MID,
                 0,
                 DISPLAY_CALL_QR_FULLSCREEN_TOP);
    lv_obj_clear_flag(s_call_qr_overlay_qrcode, LV_OBJ_FLAG_CLICKABLE | LV_OBJ_FLAG_SCROLLABLE);
#else
    display_create_figma_text(s_call_qr_overlay,
                              "QR",
                              0,
                              102,
                              DISPLAY_DRIVER_WIDTH,
    ...
    s_call_qr_overlay_device_id_label = lv_label_create(s_call_qr_overlay);
    lv_obj_set_pos(s_call_qr_overlay_device_id_label,
                   0,
                   DISPLAY_CALL_QR_FULLSCREEN_LABEL_TOP);
    lv_obj_set_width(s_call_qr_overlay_device_id_label, DISPLAY_DRIVER_WIDTH);
    lv_label_set_long_mode(s_call_qr_overlay_device_id_label, LV_LABEL_LONG_CLIP);
    lv_obj_set_style_text_align(s_call_qr_overlay_device_id_label, LV_TEXT_ALIGN_CENTER, 0);
    lv_obj_set_style_text_font(s_call_qr_overlay_device_id_label, display_ascii_font(18), 0);
    display_text_set_color(s_call_qr_overlay_device_id_label, lv_color_hex(0x10243E), 0);
    display_text_set(s_call_qr_overlay_device_id_label, s_call_qr_payload);
    lv_obj_clear_flag(s_call_qr_overlay_device_id_label,
                                   ...
                                   lv_coord_t size)
{
    ...
    lv_obj_add_event_cb(qr_box, display_call_qr_tap_cb, LV_EVENT_CLICKED, NULL);
#if LV_USE_QRCODE
    ...
    s_call_device_id_label = lv_label_create(parent);
    lv_obj_set_pos(s_call_device_id_label, x, y + size + 4);
    lv_obj_set_width(s_call_device_id_label, size);
    lv_label_set_long_mode(s_call_device_id_label, LV_LABEL_LONG_CLIP);
    lv_obj_set_style_text_align(s_call_device_id_label, LV_TEXT_ALIGN_CENTER, 0);
    lv_obj_set_style_text_font(s_call_device_id_label, display_ascii_font(12), 0);
    display_text_set_color(s_call_device_id_label, lv_color_hex(0x10243E), 0);
    display_text_set(s_call_device_id_label, "");
    lv_obj_clear_flag(s_call_device_id_label, LV_OBJ_FLAG_CLICKABLE | LV_OBJ_FLAG_SCROLLABLE);
}
                                                   ...
                                                   display_call_add_field_t field)
{
    ...
    lv_obj_t *value_label = display_create_figma_text(
        row,
        value[0] != '\0' ? value : display_call_add_field_placeholder(field),
        85,
        13,
        176,
        value[0] != '\0' ? lv_color_hex(0x10233B) : lv_color_hex(0x8AA0B5),
        12,
        LV_TEXT_ALIGN_LEFT);
    display_create_figma_text(row,
                              ">",
...
static lv_obj_t *display_create_wechat_add_field_row(lv_obj_t *parent, lv_coord_t y)
{
    ...
    lv_obj_t *value_label = display_create_figma_text(
        row,
        s_wechat_add_open_id[0] != '\0' ? s_wechat_add_open_id : "28位微信Open ID",
        85,
        13,
        176,
        s_wechat_add_open_id[0] != '\0' ? lv_color_hex(0x10233B) : lv_color_hex(0x8AA0B5),
        12,
        LV_TEXT_ALIGN_LEFT);
    display_create_figma_text(row,
                              ">",
...
static void display_create_call_contact_row(lv_obj_t *parent, uint8_t index, lv_coord_t y)
{
    ...
    lv_color_t button_fill = online ? lv_color_hex(0xDDF8EA) : lv_color_hex(0xEEF3F7);
    lv_color_t button_text = online ? lv_color_hex(0x1FC985) : lv_color_hex(0x8AA0B5);
    const char *subtitle = s_call_contacts[index].remark[0] != '\0' ?
        s_call_contacts[index].remark :
        (s_call_contacts[index].online ? "Online" : "Offline");
    lv_obj_t *call_btn = NULL;
    lv_obj_t *remark_btn = NULL;
    lv_obj_t *touch_target = NULL;
    lv_obj_t *row = display_create_figma_box(parent,
                                             8,
                                             y,
                                             304,
    ...
    remark_btn = display_create_figma_button(row,
                                             164,
                                             7,
                                             54,
                                             30,
    ...
    lv_obj_clear_flag(call_btn, LV_OBJ_FLAG_CLICKABLE | LV_OBJ_FLAG_SCROLLABLE);
    s_call_contact_call_buttons[index] = call_btn;

    touch_target = lv_obj_create(row);
    lv_obj_remove_style_all(touch_target);
    lv_obj_set_pos(touch_target, DISPLAY_CALL_CONTACT_TOUCH_LEFT, 0);
    lv_obj_set_size(touch_target,
                    DISPLAY_CALL_CONTACT_TOUCH_WIDTH,
                    DISPLAY_CALL_CONTACT_TOUCH_HEIGHT);
    lv_obj_set_style_bg_opa(touch_target, LV_OPA_TRANSP, 0);
    lv_obj_add_flag(touch_target, LV_OBJ_FLAG_CLICKABLE);
    lv_obj_clear_flag(touch_target, LV_OBJ_FLAG_SCROLLABLE);
    lv_obj_add_event_cb(touch_target,
                        display_call_contact_touch_cb,
                        LV_EVENT_ALL,
                        (void *)(uintptr_t)index);
    lv_obj_add_event_cb(touch_target,
                        display_call_contact_delete_cb,
...
static void display_create_call_scan_info_overlay(lv_obj_t *parent)
{
    ...
    lv_obj_set_height(format_label, 20);
...
static void display_show_call_remark_page(uint8_t contact_index)
{
    ...
    if (s_call_remark_status_label != NULL) {
        ...
        display_text_set(s_call_remark_status_label, "点击保存生效");
    }
...
static void display_show_wechat_add_edit_page(void)
{
    ...
    display_update_wechat_add_edit_feedback(NULL, lv_color_hex(0x0D8A59));
    display_show_text_keyboard(s_wechat_add_edit_keyboard, s_wechat_add_edit_ta);
}
...
static void display_show_wifi_connect_page(void)
{
    ...
    display_set_wifi_connect_state(DISPLAY_WIFI_CONNECT_STATE_IDLE);
...
static void display_build_binding_code_dialog(lv_obj_t *overlay)
{
    ...
    if (lv_qrcode_update(s_binding_platform_qrcode,
                          binding_platform_url,
                          strlen(binding_platform_url)) != LV_RES_OK) {
...
static void display_update_binding_prompt(const display_status_t *status)
{
    ...
    bound = status->binding_state == DISPLAY_DEVICE_BINDING_STATE_BOUND ||
            (status->binding_state == DISPLAY_DEVICE_BINDING_STATE_DISABLED &&
...
static esp_err_t display_request_call_contacts_refresh(const char *source)
{
    ...
    if (s_actions.on_refresh_call_contacts != NULL) {
        ...
        if (ret != ESP_OK && ret != ESP_ERR_INVALID_STATE) {
            ESP_LOGW(TAG,
                     "refresh call contacts on list entry failed: %s",
                     esp_err_to_name(ret));
        }
    }
...
static void display_call_list_refresh_btn_cb(lv_event_t *event)
{
    ...
    if (code == LV_EVENT_PRESSED ||
        ...
        code == LV_EVENT_PRESS_LOST) {
        image = lv_obj_get_child(lv_event_get_target(event), 0);
        if (image != NULL) {
...
static void display_call_confirm_add_btn_cb(lv_event_t *event)
{
    ...
    if (!display_text_has_visible_char(device_id_trimmed)) {
        display_show_wifi_alert("Add Contact", "Device ID is required.");
        return;
    }
    if (strlen(device_id_trimmed) != DISPLAY_CALL_CONTACT_DEVICE_ID_LENGTH) {
        display_show_wifi_alert("Add Contact", "Device ID must be 12 characters.");
        return;
    }

    display_hide_keyboard();
    if (s_actions.on_add_call_contact == NULL) {
        display_show_wifi_alert("Add Contact", "Contact service is unavailable.");
        return;
    }
...
static void display_call_contact_call_btn_cb(lv_event_t *event)
{
    ...
    esp_err_t ret = ESP_OK;
    bool valid_contact = contact_index < s_call_contact_count &&
                         s_call_contacts[contact_index].device_id[0] != '\0';

    ESP_LOGI(CALL_FLOW_TAG,
             "stage=ui_call_tap index=%u count=%u peer=%s online=%d",
             (unsigned)contact_index,
             (unsigned)s_call_contact_count,
             valid_contact ? s_call_contacts[contact_index].device_id : "-",
             valid_contact && s_call_contacts[contact_index].online ? 1 : 0);
    ...
    if (ret != ESP_OK) {
        display_show_wifi_alert("呼叫",
                                ret == ESP_ERR_INVALID_STATE && !s_last_status.network_connected ?
                                "请先连接 Wi-Fi" : "呼叫启动失败");
        return;
    }
    s_call_active_started_us = 0;
    s_call_terminal_visible_until_us = 0;
    s_call_page_entry_state = DISPLAY_CALL_STATE_OUTGOING;
    s_call_page_entry_role = DISPLAY_CALL_ROLE_CALLER;
    strlcpy(s_call_active_peer_id,
            s_call_contacts[contact_index].device_id,
            sizeof(s_call_active_peer_id));
    display_show_call_active_page();
...
static void display_call_contact_remark_btn_cb(lv_event_t *event)
{
    ...
    if (code == LV_EVENT_PRESSED) {
        if (contact_index < DISPLAY_CALL_CONTACT_COUNT) {
            s_call_contact_long_pressed[contact_index] = false;
        }
        return;
    }
    if (code != LV_EVENT_CLICKED) {
    ...
    if (contact_index < DISPLAY_CALL_CONTACT_COUNT &&
        s_call_contact_long_pressed[contact_index]) {
        s_call_contact_long_pressed[contact_index] = false;
        return;
    }
    display_show_call_remark_page(contact_index);
}
...
static void display_call_contact_delete_cb(lv_event_t *event)
{
    ...
    uint8_t contact_index = (uint8_t)(uintptr_t)lv_event_get_user_data(event);
    if (contact_index < DISPLAY_CALL_CONTACT_COUNT) {
        s_call_contact_long_pressed[contact_index] = true;
        if (s_call_contact_call_buttons[contact_index] != NULL) {
            lv_obj_clear_state(s_call_contact_call_buttons[contact_index], LV_STATE_PRESSED);
        }
    }
    lv_event_stop_bubbling(event);
...
static void display_call_delete_confirm_btn_cb(lv_event_t *event)
{
    ...
    if (ret != ESP_OK) {
        ESP_LOGW(CALL_FLOW_TAG,
                 "stage=contact_delete_rejected peer=%s ret=%s",
                 device_id,
                 esp_err_to_name(ret));
        const char *message = ret == ESP_ERR_NOT_ALLOWED ?
            "Same-account contact\nReset device on Web" :
            (ret == ESP_ERR_INVALID_STATE ? "联系人服务忙，请稍后再试" : "删除失败");
        display_show_wifi_alert("联系人", message);
        return;
    }
    ESP_LOGI(CALL_FLOW_TAG, "stage=contact_delete_queued peer=%s", device_id);
    display_show_wifi_alert("删除联系人", "Deleting...");
}
...
static void display_call_remark_save_btn_cb(lv_event_t *event)
{
    ...
    display_copy_trimmed_text(remark,
                              sizeof(remark),
                              input);
...
static void display_call_contact_touch_cb(lv_event_t *event)
{
    uint8_t contact_index = (uint8_t)(uintptr_t)lv_event_get_user_data(event);
    lv_event_code_t code = lv_event_get_code(event);
    lv_obj_t *visual_button = contact_index < DISPLAY_CALL_CONTACT_COUNT ?
        s_call_contact_call_buttons[contact_index] : NULL;
    ...
    if (code == LV_EVENT_PRESSED) {
        ...
        if (visual_button != NULL) {
            lv_obj_add_state(visual_button, LV_STATE_PRESSED);
        }
        return;
    }
    if (code == LV_EVENT_RELEASED || code == LV_EVENT_PRESS_LOST) {
        if (visual_button != NULL) {
            lv_obj_clear_state(visual_button, LV_STATE_PRESSED);
        }
        return;
    }
    if (code == LV_EVENT_CLICKED) {
        ...
        display_call_contact_call_btn_cb(event);
    }
}
...
static void display_call_hangup_btn_cb(lv_event_t *event)
{
    ...
    s_call_active_started_us = 0;
    strlcpy(s_call_terminal_message, "call ended", sizeof(s_call_terminal_message));
    s_call_terminal_visible_until_us = esp_timer_get_time() + DISPLAY_CALL_TERMINAL_HOLD_US;
    s_call_page_entry_state = DISPLAY_CALL_STATE_IDLE;
    s_call_page_entry_role = DISPLAY_CALL_ROLE_NONE;
    display_update_call_active_page(&s_last_status);
}
...
static void display_wechat_confirm_add_btn_cb(lv_event_t *event)
{
    ...
    if (ret != ESP_OK) {
        display_show_wifi_alert("微信联系人",
                                ret == ESP_ERR_INVALID_STATE ? "授权列表同步中，请稍后重试" :
                                ret == ESP_ERR_NOT_FOUND ? "该用户尚未授权此设备" :
                                ret == ESP_ERR_NO_MEM ? "内存不足" : "验证失败");
        return;
    }

    display_reset_wechat_add_input();
    display_show_wechat_list_page();
    display_show_wifi_alert("微信联系人", "该联系人已在授权列表");
}
...
static void display_wechat_delete_confirm_btn_cb(lv_event_t *event)
{
    ...
    display_hide_wechat_delete_confirm();
    display_show_wifi_alert("微信联系人", "请在微信小程序取消授权");
}
...
static void display_tirtc_config_scan_btn_cb(lv_event_t *event)
{
    ...
    if (ret == ESP_OK) {
        /* Keep the last real snapshot until the forced token check confirms
         * cloud unbind with 6006.  The binding service owns the transition to
         * REPORTING and the new verification code; the UI must not invent it.
         */
        display_return_home();
        return;
    }
    ...
    case DISPLAY_DEVICE_VOLUME_SEND_MUTE:
        if (current > 0U) {
            ...
            next = s_device_receive_restore_valid
                       ...
                       : DISPLAY_DEVICE_SPEAKER_VOLUME_RESTORE_DEFAULT;
        } else {
            next = s_device_send_restore_valid
                       ? s_device_send_restore_volume
                       : DISPLAY_DEVICE_CAPTURE_GAIN_RESTORE_DEFAULT;
        }
        break;
    default:
        return;
...
static void display_update_wifi_scan_state(const display_status_t *status)
{
    ...
    if (s_wifi_connect_pending) {
        left_color = lv_color_hex(0xF59E0B);
        if (s_wifi_connect_target_ssid[0] != '\0') {
            snprintf(left_text, sizeof(left_text), "连接中 %s", s_wifi_connect_target_ssid);
        } else {
            strlcpy(left_text, "连接中", sizeof(left_text));
        }
        use_runtime_left = true;
    } else if (status->network_connected) {
    ...
    if (use_runtime_left && s_wifi_connection_runtime_label != NULL) {
        lv_obj_add_flag(s_wifi_connection_state_label, LV_OBJ_FLAG_HIDDEN);
        display_text_set_color(s_wifi_connection_runtime_label, left_color, 0);
        display_text_set(s_wifi_connection_runtime_label, left_text);
        lv_obj_clear_flag(s_wifi_connection_runtime_label, LV_OBJ_FLAG_HIDDEN);
    } else {
        if (s_wifi_connection_runtime_label != NULL) {
            lv_obj_add_flag(s_wifi_connection_runtime_label, LV_OBJ_FLAG_HIDDEN);
        }
...
static void display_update_main_page(const display_status_t *status)
{
    ...
    if (s_device_receive_volume_label != NULL) {
        display_text_set_uint(s_device_receive_volume_label,
                              status->audio_speaker_volume_percent);
    }
    ...
    if (s_device_send_volume_label != NULL) {
        display_text_set_uint(s_device_send_volume_label,
                              status->audio_capture_gain_percent);
    }
...
static void display_update_ai_chat_page(const display_status_t *status)
{
    ...
    reply_active = status->ai_chat_cloud_speaking ||
                   status->ai_chat_output_playback_pending;

    if (show_new_chat_button) {
        state_text = "休息";
        state_color = lv_color_hex(0x64758A);
    } else if (status->ai_chat_listening && !reply_active) {
        state_text = "聆听";
        state_color = lv_color_hex(0x2F82D7);
    } else if (reply_active) {
        state_text = "回复";
    ...
    if (latest_message != NULL) {
        ...
        caption_color = latest_message->caption_type == DISPLAY_AI_CHAT_CAPTION_TYPE_ASR ?
            ...
            lv_color_hex(0x1768B7);
    } else if (reply_active && status->ai_chat_tts_caption[0] != '\0') {
...
static void display_update_ai_chat_settings_page(const display_status_t *status)
{
    ...
    if (s_ai_settings_mic_value_label != NULL) {
        display_text_set_uint(s_ai_settings_mic_value_label,
                              status->audio_capture_gain_percent);
    }

    if (s_ai_settings_speaker_value_label != NULL) {
        display_text_set_uint(s_ai_settings_speaker_value_label,
                              status->audio_speaker_volume_percent);
    }
...
static void display_update_call_page(const display_status_t *status)
{
    ...
    if (s_call_qr_overlay_device_id_label != NULL) {
        display_text_set(s_call_qr_overlay_device_id_label, payload);
    }
...
#if LV_USE_QRCODE
    if (s_call_qrcode != NULL && display_update_device_qr_object(s_call_qrcode, payload)) {
        updated = true;
    } else if (s_call_qrcode != NULL) {
        ESP_LOGW(TAG, "call qr update failed");
    }
    if (s_call_qr_overlay_qrcode != NULL &&
        display_update_device_qr_object(s_call_qr_overlay_qrcode, payload)) {
        updated = true;
    } else if (s_call_qr_overlay_qrcode != NULL) {
        ESP_LOGW(TAG, "call qr fullscreen update failed");
    }
    if (updated) {
...
static void display_call_set_visible(lv_obj_t *obj, bool visible)
{
    if (obj == NULL) {
        return;
    }
...
static const char *display_call_peer_name(const char *peer_device_id)
{
    if (peer_device_id != NULL && peer_device_id[0] != '\0') {
        for (uint8_t index = 0; index < s_call_contact_count; ++index) {
            if (strcmp(s_call_contacts[index].device_id, peer_device_id) == 0 &&
                s_call_contacts[index].remark[0] != '\0') {
                return s_call_contacts[index].remark;
            }
        }
    }
    return "设备联系人";
...
static const char *display_call_terminal_title(const char *message, int last_error)
{
    if (message != NULL) {
        if (strcmp(message, "peer hung up") == 0) {
            return "对方已挂断";
        }
        if (strcmp(message, "incoming call canceled") == 0) {
            return "对方已取消";
        }
        if (strcmp(message, "call timed out") == 0) {
            return "暂时无人接听";
        }
        if (strcmp(message, "peer connection timed out") == 0) {
            return "连接超时";
        }
        if (strcmp(message, "RTC disconnected") == 0) {
            return "连接已断开";
        }
        if (strcmp(message, "call rejected") == 0) {
            return "对方已拒绝";
        }
        if (strcmp(message, "call ended") == 0) {
            return "通话已结束";
        }
    }
    return last_error == ESP_OK ? "通话已结束" : "呼叫失败";
}

static void display_call_begin_terminal_feedback(const display_status_t *status, int64_t now_us)
{
    const char *message = status != NULL ? status->call_message : "";

    strlcpy(s_call_terminal_message,
            message != NULL && message[0] != '\0' ? message : "call ended",
            sizeof(s_call_terminal_message));
    s_call_terminal_visible_until_us = now_us + DISPLAY_CALL_TERMINAL_HOLD_US;
    s_call_page_entry_state = DISPLAY_CALL_STATE_IDLE;
    s_call_page_entry_role = DISPLAY_CALL_ROLE_NONE;
}
...
static void display_update_call_active_page(const display_status_t *status)
{
    display_call_state_t state = DISPLAY_CALL_STATE_IDLE;
    display_call_role_t role = DISPLAY_CALL_ROLE_NONE;
    const char *peer_id = NULL;
    const char *state_text = "正在呼叫";
    const char *detail_text = "正在发起呼叫...";
    uint8_t mic = 0;
    uint8_t speaker = 0;
    int64_t now_us = esp_timer_get_time();
    int64_t elapsed_seconds = 0;
    char duration[8] = "00:00";
    bool terminal_feedback = s_call_terminal_visible_until_us > now_us;
    bool in_call = false;
    bool active = false;
    ...
    state = status->call_state;
    role = status->call_role;
    if (!terminal_feedback &&
        state != DISPLAY_CALL_STATE_OUTGOING &&
        state != DISPLAY_CALL_STATE_INCOMING &&
        state != DISPLAY_CALL_STATE_CONNECTING &&
        state != DISPLAY_CALL_STATE_IN_CALL &&
        s_call_page_entry_state != DISPLAY_CALL_STATE_IDLE) {
        state = s_call_page_entry_state;
        role = s_call_page_entry_role;
    }
    if (status->call_state == DISPLAY_CALL_STATE_OUTGOING ||
        status->call_state == DISPLAY_CALL_STATE_INCOMING ||
        status->call_state == DISPLAY_CALL_STATE_CONNECTING ||
        status->call_state == DISPLAY_CALL_STATE_IN_CALL) {
        s_call_page_entry_state = DISPLAY_CALL_STATE_IDLE;
        s_call_page_entry_role = DISPLAY_CALL_ROLE_NONE;
    }
    ...
    if (status->call_peer_device_id[0] != '\0') {
        strlcpy(s_call_active_peer_id,
                status->call_peer_device_id,
                sizeof(s_call_active_peer_id));
    }
    peer_id = s_call_active_peer_id[0] != '\0' ? s_call_active_peer_id : "未知设备";

    in_call = !terminal_feedback && state == DISPLAY_CALL_STATE_IN_CALL;
    active = !terminal_feedback &&
             (state == DISPLAY_CALL_STATE_OUTGOING ||
              state == DISPLAY_CALL_STATE_INCOMING ||
              state == DISPLAY_CALL_STATE_CONNECTING ||
              state == DISPLAY_CALL_STATE_IN_CALL);
    ...
    if (terminal_feedback) {
        state_text = display_call_terminal_title(s_call_terminal_message,
                                                 status->call_last_error);
        detail_text = "即将返回呼叫页面";
    } else {
        switch (state) {
        case DISPLAY_CALL_STATE_OUTGOING:
            state_text = "正在呼叫";
            detail_text = strcmp(status->call_message, "requesting call") == 0 ?
                          "正在发起呼叫..." : "等待对方接听...";
            break;
        ...
        case DISPLAY_CALL_STATE_CONNECTING:
            state_text = role == DISPLAY_CALL_ROLE_CALLEE ? "接听中" : "对方已接听";
            detail_text = "正在建立音频连接...";
            break;
        ...
        case DISPLAY_CALL_STATE_ERROR:
            state_text = display_call_terminal_title(status->call_message,
                                                     status->call_last_error);
            detail_text = "请稍后重试";
            break;
    ...
    display_call_set_visible(s_call_status_card, !in_call);
    display_call_set_visible(s_call_duration_row, in_call);
    display_call_set_visible(s_call_mic_row, in_call);
    display_call_set_visible(s_call_speaker_row, in_call);
    display_call_set_visible(s_call_hangup_btn, active);

    if (s_call_peer_name_label != NULL) {
        display_text_set(s_call_peer_name_label, display_call_peer_name(peer_id));
    }
    if (s_call_peer_id_label != NULL) {
        display_text_set(s_call_peer_id_label, peer_id);
    }
    if (s_call_state_label != NULL) {
        display_text_set(s_call_state_label, state_text);
        display_text_set_color(s_call_state_label,
                               terminal_feedback || state == DISPLAY_CALL_STATE_ERROR ?
                                   lv_color_hex(0xE44747) : lv_color_hex(0x20A96B),
                               0);
    }
    if (s_call_state_detail_label != NULL) {
        display_text_set(s_call_state_detail_label, detail_text);
    }

    if (in_call && s_call_active_started_us == 0) {
        s_call_active_started_us = now_us;
    }
    if (!in_call) {
        s_call_active_started_us = 0;
    }
    if (s_call_active_started_us > 0) {
        elapsed_seconds = (now_us - s_call_active_started_us) / 1000000LL;
    ...
    if (s_call_mic_value_label != NULL) {
        display_text_set_uint(s_call_mic_value_label, mic);
    }
    if (s_call_speaker_value_label != NULL) {
        display_text_set_uint(s_call_speaker_value_label, speaker);
    }
}
...
static void display_update_wechat_active_page(const display_status_t *status)
{
    ...
    if (s_wechat_mic_value_label != NULL) {
        display_text_set_uint(s_wechat_mic_value_label, mic);
    }
    if (s_wechat_speaker_value_label != NULL) {
        display_text_set_uint(s_wechat_speaker_value_label, speaker);
    }
}

static void display_update_wifi_connect_feedback(const display_status_t *status)
{
    const char *connect_target = s_wifi_connect_target_ssid[0] != '\0'
                                     ? s_wifi_connect_target_ssid
                                     : s_selected_ssid;
    bool connect_target_matched = status->network_connected &&
                                  (connect_target[0] == '\0' ||
                                   status->network_ssid[0] == '\0' ||
                                   strcmp(status->network_ssid, connect_target) == 0);

    if (connect_target_matched) {
        s_wifi_connect_pending = false;
        display_set_wifi_connect_state(DISPLAY_WIFI_CONNECT_STATE_CONNECTED);
    ...
    if (status->network_connect_failed) {
        s_wifi_connect_pending = false;
        switch (status->network_connect_failure) {
        case NETWORK_CONNECT_FAILURE_AUTHENTICATION:
            display_set_wifi_connect_state(DISPLAY_WIFI_CONNECT_STATE_AUTH_FAILED);
            break;
        case NETWORK_CONNECT_FAILURE_AP_NOT_FOUND:
            display_set_wifi_connect_state(DISPLAY_WIFI_CONNECT_STATE_AP_NOT_FOUND);
            break;
        case NETWORK_CONNECT_FAILURE_TIMEOUT:
            display_set_wifi_connect_state(DISPLAY_WIFI_CONNECT_STATE_TIMEOUT);
            break;
    ...
    /* Connection failure ownership lives in the Wi-Fi state machine. A UI
     * timer cannot distinguish AP association from DHCP and used to report a
     * false timeout while the station was already waiting for an address. */
}
...
static void display_build_system_page(lv_obj_t *screen)
{
    ...
    display_create_settings_row(s_system_page, 156, "关于 / OTA", display_system_ota_btn_cb);
}
...
static void display_build_call_page(lv_obj_t *screen)
{
    ...
    display_create_call_qr(qr_card,
                           (192 - DISPLAY_CALL_QR_SIZE) / 2,
                           4,
                           DISPLAY_CALL_QR_SIZE);
}
...
static void display_build_call_add_edit_page(lv_obj_t *screen)
{
    ...
    lv_obj_set_align(s_call_add_edit_keyboard, LV_ALIGN_TOP_LEFT);
...
static void display_build_call_list_page(lv_obj_t *screen)
{
    ...
    header = display_create_figma_header_base(s_call_list_page,
                                              "联系人列表",
                                              display_call_child_back_btn_cb);
    display_create_call_list_refresh_button(header);

    s_call_list_content = lv_obj_create(s_call_list_page);
    lv_obj_remove_style_all(s_call_list_content);
    lv_obj_set_pos(s_call_list_content, 0, 30);
    lv_obj_set_size(s_call_list_content, DISPLAY_DRIVER_WIDTH, DISPLAY_DRIVER_HEIGHT - 30);
    lv_obj_set_style_bg_opa(s_call_list_content, LV_OPA_TRANSP, 0);
    lv_obj_set_style_pad_all(s_call_list_content, 0, 0);
...
static void display_build_call_remark_page(lv_obj_t *screen)
{
    ...
    (void)display_create_figma_text(s_call_remark_page,
                                    "联系人名称",
                                    8,
                                    36,
                                    196,
                                    ...
                                    12,
                                    LV_TEXT_ALIGN_LEFT);
    s_call_remark_length_label = display_create_figma_text(s_call_remark_page,
                                                            "0/63",
                                                            230,
                                                            36,
                                                            82,
    ...
    lv_obj_set_pos(s_call_remark_ta, 8, DISPLAY_UUID_INPUT_TOP);
    lv_obj_set_size(s_call_remark_ta, DISPLAY_UUID_INPUT_WIDTH, DISPLAY_UUID_INPUT_HEIGHT);
    ...
    lv_obj_set_style_text_font(s_call_remark_ta, display_runtime_utf8_font(), 0);
    lv_obj_set_style_pad_left(s_call_remark_ta, 12, 0);
    lv_obj_set_style_pad_right(s_call_remark_ta, 12, 0);
    lv_obj_set_style_pad_top(s_call_remark_ta, 2, 0);
    lv_obj_set_style_pad_bottom(s_call_remark_ta, 2, 0);
    lv_obj_add_event_cb(s_call_remark_ta,
                        display_call_remark_textarea_event_cb,
                        LV_EVENT_ALL,
                        NULL);

    s_call_remark_status_label = display_create_figma_text(s_call_remark_page,
                                                            "点击保存生效",
                                                            8,
                                                            DISPLAY_UUID_STATUS_TOP,
                                                            DISPLAY_UUID_STATUS_WIDTH,
    ...
    lv_obj_set_pos(s_call_remark_keyboard, DISPLAY_UUID_KEYBOARD_LEFT, DISPLAY_UUID_KEYBOARD_TOP);
    lv_obj_set_size(s_call_remark_keyboard, DISPLAY_UUID_KEYBOARD_WIDTH, DISPLAY_UUID_KEYBOARD_HEIGHT);
    lv_obj_set_style_bg_opa(s_call_remark_keyboard, LV_OPA_TRANSP, 0);
    lv_obj_set_style_pad_all(s_call_remark_keyboard, 0, 0);
    lv_obj_set_style_pad_row(s_call_remark_keyboard, 3, 0);
    lv_obj_set_style_pad_column(s_call_remark_keyboard, 2, 0);
...
static void display_build_call_active_page(lv_obj_t *screen)
{
    ...
    s_call_status_card = display_create_figma_box(s_call_active_page,
                                                  8,
                                                  38,
                                                  304,
                                                  122,
                                                  ...
                                                  lv_color_hex(0xD6E4EF),
                                                  8);
    s_call_peer_name_label = display_create_ai_dialog_text(s_call_status_card,
                                                           "设备联系人",
                                                           16,
                                                           12,
                                                           272,
                                                           lv_color_hex(0x10233B),
                                                           LV_TEXT_ALIGN_CENTER);
    s_call_peer_id_label = display_create_figma_text(s_call_status_card,
                                                     "未知设备",
                                                     16,
                                                     36,
                                                     272,
                                                     ...
                                                     12,
                                                     LV_TEXT_ALIGN_CENTER);
    s_call_state_label = display_create_ai_dialog_text(s_call_status_card,
                                                       "正在呼叫",
                                                       16,
                                                       64,
                                                       272,
                                                       lv_color_hex(0x20A96B),
                                                       LV_TEXT_ALIGN_CENTER);
    s_call_state_detail_label = display_create_ai_dialog_text(s_call_status_card,
                                                              "正在发起呼叫...",
                                                              16,
                                                              90,
                                                              272,
    ...
    s_call_duration_row = display_create_call_duration_row(s_call_active_page,
                                                           38,
                                                           &s_call_duration_label);
    s_call_mic_row = display_create_call_volume_row(s_call_active_page,
                                                    80,
                                                    "麦克风",
                                                    "62",
    ...
    s_call_speaker_row = display_create_call_volume_row(s_call_active_page,
                                                        124,
                                                        "扬声器",
                                                        "70",
                                                        DISPLAY_CALL_VOLUME_SPEAKER_DOWN,
                                                        ...
                                                        display_call_volume_btn_cb);
    s_call_hangup_btn = display_create_figma_button(s_call_active_page,
                                                    8,
                                                    168,
                                                    304,
                                                    38,
...
static void display_build_wechat_add_edit_page(lv_obj_t *screen)
{
    ...
    lv_obj_set_style_text_font(s_wechat_add_edit_ta, display_runtime_utf8_font(), 0);
    lv_obj_set_style_pad_left(s_wechat_add_edit_ta, 12, 0);
    lv_obj_set_style_pad_right(s_wechat_add_edit_ta, 12, 0);
    lv_obj_set_style_pad_top(s_wechat_add_edit_ta, 2, 0);
    lv_obj_set_style_pad_bottom(s_wechat_add_edit_ta, 2, 0);
...
static void display_build_wechat_remark_page(lv_obj_t *screen)
{
    ...
    lv_obj_set_pos(s_wechat_remark_ta, 8, DISPLAY_UUID_INPUT_TOP);
    lv_obj_set_size(s_wechat_remark_ta, DISPLAY_UUID_INPUT_WIDTH, DISPLAY_UUID_INPUT_HEIGHT);
    lv_textarea_set_one_line(s_wechat_remark_ta, true);
    lv_textarea_set_max_length(s_wechat_remark_ta,
                               (DISPLAY_WECHAT_REMARK_MAX - 1U) / 4U);
    ...
    lv_obj_set_style_text_font(s_wechat_remark_ta, display_runtime_utf8_font(), 0);
    ...
    lv_obj_set_pos(s_wechat_remark_keyboard,
                   DISPLAY_UUID_KEYBOARD_LEFT,
                   DISPLAY_UUID_KEYBOARD_TOP);
    lv_obj_set_size(s_wechat_remark_keyboard,
                    DISPLAY_UUID_KEYBOARD_WIDTH,
    ...
    lv_obj_set_style_pad_row(s_wechat_remark_keyboard, 3, 0);
    lv_obj_set_style_pad_column(s_wechat_remark_keyboard, 2, 0);
    ...
    lv_obj_add_event_cb(s_wechat_remark_keyboard,
                        ...
                        NULL);
    lv_obj_add_flag(s_wechat_remark_keyboard, LV_OBJ_FLAG_HIDDEN);
    lv_obj_add_event_cb(s_wechat_remark_keyboard, display_keyboard_event_cb, LV_EVENT_ALL, NULL);
}
...
static void display_build_tirtc_config_edit_page(lv_obj_t *screen)
{
    ...
    lv_obj_set_align(s_tirtc_edit_keyboard, LV_ALIGN_TOP_LEFT);
...
static void display_build_wifi_page(lv_obj_t *screen)
{
    ...
    s_wifi_connection_state_label = display_create_figma_text(s_wifi_page,
                                                                     ...
                                                                     lv_color_hex(0x64758A),
                                                                      12,
                                                                      LV_TEXT_ALIGN_LEFT);
    s_wifi_connection_runtime_label = lv_label_create(s_wifi_page);
    if (s_wifi_connection_runtime_label != NULL) {
        lv_obj_set_pos(s_wifi_connection_runtime_label, 8, 34);
        lv_obj_set_width(s_wifi_connection_runtime_label, 210);
        lv_label_set_long_mode(s_wifi_connection_runtime_label, LV_LABEL_LONG_DOT);
        lv_obj_set_style_text_align(s_wifi_connection_runtime_label, LV_TEXT_ALIGN_LEFT, 0);
        lv_obj_set_style_text_font(s_wifi_connection_runtime_label, display_runtime_utf8_font(), 0);
        display_text_set_color(s_wifi_connection_runtime_label, lv_color_hex(0x64758A), 0);
        display_text_set(s_wifi_connection_runtime_label, "");
        lv_obj_add_flag(s_wifi_connection_runtime_label, LV_OBJ_FLAG_HIDDEN);
    }
    ...
    for (uint16_t index = 0; index < DISPLAY_WIFI_SCAN_MAX; ++index) {
        ...
        /* SSIDs are runtime UTF-8 data, so bitmap text assets and ASCII fonts cannot render them. */
        lv_obj_set_style_text_font(label_row, display_runtime_utf8_font(), 0);
...
static void display_build_wifi_connect_page(lv_obj_t *screen)
{
    ...
    s_wifi_connect_hint_label = display_create_figma_text(s_wifi_connect_page,
                                                                 ...
                                                                 12,
                                                                 LV_TEXT_ALIGN_LEFT);
    s_wifi_connect_progress_label = lv_label_create(s_wifi_connect_page);
    if (s_wifi_connect_progress_label != NULL) {
        lv_obj_set_pos(s_wifi_connect_progress_label,
                       DISPLAY_WIFI_CONNECT_HINT_LEFT,
                       DISPLAY_WIFI_CONNECT_HINT_TOP - 3);
        lv_obj_set_width(s_wifi_connect_progress_label, DISPLAY_WIFI_CONNECT_HINT_WIDTH);
        lv_label_set_long_mode(s_wifi_connect_progress_label, LV_LABEL_LONG_DOT);
        lv_obj_set_style_text_align(s_wifi_connect_progress_label, LV_TEXT_ALIGN_LEFT, 0);
        lv_obj_set_style_text_font(s_wifi_connect_progress_label, display_runtime_utf8_font(), 0);
        display_text_set_color(s_wifi_connect_progress_label, lv_color_hex(0xF59E0B), 0);
        display_text_set(s_wifi_connect_progress_label, "");
        lv_obj_add_flag(s_wifi_connect_progress_label, LV_OBJ_FLAG_HIDDEN);
    }
    s_wifi_connect_rssi_label = display_create_figma_text(
        s_wifi_connect_page,
        "",
...
static void display_refresh_timer(lv_timer_t *timer)
{
    ...
    *previous_status = s_last_status;
    ...
    bool binding_reconcile_requires_home =
        status->binding_state == DISPLAY_DEVICE_BINDING_STATE_REPORTING ||
        status->binding_state == DISPLAY_DEVICE_BINDING_STATE_WAITING_USER ||
        (previous_status->binding_state == DISPLAY_DEVICE_BINDING_STATE_BOUND &&
         status->binding_state != DISPLAY_DEVICE_BINDING_STATE_BOUND &&
         status->binding_state != DISPLAY_DEVICE_BINDING_STATE_DISABLED);
    if (binding_reconcile_requires_home && !display_page_is_visible(s_home_page)) {
        ESP_LOGI(TAG,
                 "binding state requires home: previous=%u current=%u",
                 (unsigned)previous_status->binding_state,
                 (unsigned)status->binding_state);
        display_return_home();
        return;
    }
    ...
    if (call_active_page_visible) {
        bool previous_call_active = display_call_state_keeps_active_page(previous_status->call_state) ||
                                    s_call_page_entry_state != DISPLAY_CALL_STATE_IDLE;
        bool current_call_active = display_call_state_keeps_active_page(status->call_state);

        if (previous_call_active && !current_call_active &&
            s_call_terminal_visible_until_us <= now_us) {
            display_call_begin_terminal_feedback(status, now_us);
        }
        display_update_call_active_page(status);
        if (!current_call_active &&
            (s_call_terminal_visible_until_us == 0 || now_us >= s_call_terminal_visible_until_us)) {
            ESP_LOGI(CALL_FLOW_TAG,
                     "stage=ui_call_page_exit state=%u message=%s",
                     (unsigned)status->call_state,
                     status->call_message[0] != '\0' ? status->call_message : "-");
            s_call_active_started_us = 0;
            s_call_terminal_visible_until_us = 0;
            s_call_terminal_message[0] = '\0';
            s_call_active_peer_id[0] = '\0';
            display_show_call_page();
        }
    }
    ...
    if (wifi_page_visible || wifi_connect_page_visible || s_wifi_connect_pending || s_last_wifi_scan_request_us > 0) {
        wifi_status_changed = !display_wifi_scan_equals(status, previous_status) ||
                              ...
                              status->network_associated != previous_status->network_associated ||
                              status->network_phase != previous_status->network_phase ||
                              status->network_connect_failed != previous_status->network_connect_failed ||
                              status->network_connect_failure != previous_status->network_connect_failure ||
        ...
        if (wifi_connect_page_visible && (wifi_status_changed || s_wifi_connect_pending)) {
...
esp_err_t display_init(const display_actions_t *actions)
{
    ...
    if (s_snapshot_provider != NULL) {
        s_snapshot_provider(&s_last_status, s_snapshot_ctx);
    }
...
static bool display_is_keyboard_object(const lv_obj_t *target)
{
    return target != NULL &&
           (target == s_keyboard ||
            ...
            target == s_wechat_remark_keyboard);
}
