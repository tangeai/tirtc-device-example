...
try {
    foreach ($size in $Sizes) {
        ...
        & $fontConv @fontConvArgs --bpp 4 --size $size @externalFontArgs --format lvgl -o $cOutFile --force-fast-kern-format
        ...
        if ($GenerateSpiffsBinary) {
            ...
            & $fontConv @fontConvArgs --bpp 4 --size $size @externalFontArgs --format bin -o $binOutFile --force-fast-kern-format
            if ($LASTEXITCODE -ne 0) {
...
$report += "external_font_compressed=true"
foreach ($size in $Sizes) {
