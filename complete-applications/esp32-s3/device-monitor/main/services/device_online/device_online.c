...
#define DEVICE_ONLINE_TASK_STACK_SIZE (24 * 1024)
...
static device_online_runtime_t s_online;
...
esp_err_t device_online_init(const device_online_config_t *config)
{
    ...
    if (s_online.lock == NULL) {
        s_online.lock = xSemaphoreCreateMutex();
        if (s_online.lock == NULL) {
            return ESP_ERR_NO_MEM;
        }
    }
    if (s_online.token_refresh_lock == NULL) {
        s_online.token_refresh_lock = xSemaphoreCreateMutex();
        if (s_online.token_refresh_lock == NULL) {
            return ESP_ERR_NO_MEM;
        }
    }
