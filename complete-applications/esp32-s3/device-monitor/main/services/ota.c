...
/*
 * OTA writes flash while SPI cache can be disabled. The OTA task stack must stay
 * in internal RAM; a PSRAM stack can trip ESP-IDF's cache-disabled stack sanity
 * check and leave the UI frozen at the last shown progress value.
 */
#define OTA_TASK_STACK_SIZE        (12 * 1024)
#define OTA_TASK_PRIORITY          3
#define OTA_TASK_ALLOC_CAPS        (MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT)
#define OTA_TASK_CORE              APP_TASK_CORE_BACKGROUND
...
esp_err_t ota_start(const char *base_url, const char *device_id)
{
    ...
    BaseType_t ok = xTaskCreatePinnedToCoreWithCaps(ota_task,
                                                    "ota",
                                                    OTA_TASK_STACK_SIZE,
                                                    task_ctx,
                                                    OTA_TASK_PRIORITY,
    ...
    if (ok != pdPASS) {
        free(task_ctx);
        ESP_LOGE(TAG,
                 "OTA task allocation failed: stack=%u internal_free=%u internal_largest=%u",
