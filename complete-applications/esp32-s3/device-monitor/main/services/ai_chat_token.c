...
#define AI_CHAT_TOKEN_CACHE_FALLBACK_US (60LL * 1000LL * 1000LL)
...
static bool ai_chat_token_config_matches(const ai_chat_config_t *lhs, const ai_chat_config_t *rhs)
{
...
static esp_err_t ai_chat_token_cache_ensure(void)
{
    if (s_join_cache.lock == NULL) {
        s_join_cache.lock = xSemaphoreCreateMutex();
        if (s_join_cache.lock == NULL) {
            return ESP_ERR_NO_MEM;
        }
    }
    if (s_join_cache.join_info == NULL) {
        s_join_cache.join_info = (ai_chat_join_info_t *)ai_chat_token_calloc_psram(1, sizeof(*s_join_cache.join_info));
        if (s_join_cache.join_info == NULL) {
            return ESP_ERR_NO_MEM;
        }
    }
...
static bool ai_chat_token_cache_take(const ai_chat_config_t *config, ai_chat_join_info_t *join_info)
{
    bool hit = false;
...
static esp_err_t ai_chat_token_cache_store(const ai_chat_config_t *config, const ai_chat_join_info_t *join_info)
{
    ESP_RETURN_ON_FALSE(config != NULL && join_info != NULL, ESP_ERR_INVALID_ARG, TAG, "invalid token cache args");
                                              ...
                                              ai_chat_join_info_t *join_info)
{
    ...
    if (peer_id_len >= sizeof(join_info->peer_id) || token_len >= sizeof(join_info->token)) {
        cJSON_Delete(root);
        ESP_LOGW(TAG,
                 "AI token response too large: peer_id_len=%u/%u token_len=%u/%u",
                 (unsigned)peer_id_len,
                 (unsigned)sizeof(join_info->peer_id),
                 (unsigned)token_len,
                 (unsigned)sizeof(join_info->token));
                                                ...
                                                ai_chat_join_info_t *join_info)
{
    ...
    ESP_LOGD(TAG, "AI Chat AI token response: status=%d body_len=%u", status, (unsigned)strlen(response));
...
esp_err_t ai_chat_token_prefetch_join(const ai_chat_config_t *config)
{
    ...
    if (ai_chat_token_cache_ensure() != ESP_OK) {
        return ESP_ERR_NO_MEM;
    }
    ...
    bool cache_ready = s_join_cache.valid &&
                       ai_chat_token_config_matches(config, &s_join_cache.config) &&
                       s_join_cache.expires_us > ai_chat_token_now_wall_us() + AI_CHAT_TOKEN_CACHE_SKEW_US;
    xSemaphoreGive(s_join_cache.lock);
    if (cache_ready) {
        return ESP_OK;
    }
