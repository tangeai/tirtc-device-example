...
| 微信联系人名称 | 输入框与屏幕键盘编辑，后台异步更新；成功后刷新服务端权威列表 |
...
- 微信联系人名称通过输入框和屏幕键盘编辑，按 `OK` 完成输入后再保存；更新后以服务端
  返回列表为权威来源。
...
- 发起设备呼叫前，呼叫请求最多等待 `12` 秒，让已经运行的房间恢复、联系人刷新或联系人
  变更任务结束；超时仍忙则拒绝本次呼叫。呼叫活跃后不再启动新的房间恢复或联系人刷新，
  避免后台 HTTPS 与 RTC 准备同时竞争；通话结束后再恢复。
...
3. 微信联系人两种列表结构、名称键盘输入、权威刷新、来电和设备侧体验版
   `wx_version_type=2` 语音呼叫。
4. 小钛连续进入退出、字幕、语音播放、联系人状态查询和呼叫动作。
5. 首次完整烧录后的设备音频呼叫准备、30 秒上线等待、50 秒 RTC 等待、45 秒响铃、状态
   提示、来电铃声停止、视频类型拒绝、拒接、取消、挂断和异常房间恢复。
