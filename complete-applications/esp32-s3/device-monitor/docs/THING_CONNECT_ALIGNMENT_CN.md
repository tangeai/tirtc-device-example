...
- TiRTC SDK 版本为 `2.3.0 mini`，BuildInfo 为 `v2.3.0-1baf7c95`，完整 commit 为
  `1baf7c95f3ca715c9367b9c998417f647934dc35`。
...
| 微信呼叫 | 小程序和设备可双向发起音频通话；能力上报明确 `no_video=true` |
| 小钛对讲 | WHIP 音频、ASR/TTS 字幕、打断和会话重建 |
| 小钛联系人动作 | 查询设备联系人状态；发起设备或微信联系人的音频呼叫 |
| 设备呼叫 | 只接受 `call_type=audio`，覆盖接听、拒接、取消、挂断和房间恢复 |
...
5. 设备音频呼叫、视频类型拒绝、拒接、取消、挂断和异常房间恢复。
