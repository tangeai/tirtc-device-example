...
```powershell
Get-FileHash .\esp32s3-tirtc-device-monitor-full-v1.9.7.bin -Algorithm SHA256
```
...
| 项目 | 值 |
| --- | --- |
| 文件 | `esp32s3-tirtc-device-monitor-full-v1.9.7.bin` |
| 大小 | `16777216` bytes |
| SHA-256 | `48a92849ef0149a4b099e981c4e109fcfa81afcbf99d379da1f7de61ad996bd1` |
...
4. 点击 `Add File`，添加 `esp32s3-tirtc-device-monitor-full-v1.9.7.bin`。
...
1. 确认文件名含 `full-v1.9.7`，不是 `ota-v1.9.7`。
2. 确认烧录地址为 `0x0`。
3. 擦除 Flash 后重新烧录。
4. 烧录结束后按 `RESET`。
...
| 文件 | 用途 |
| --- | --- |
| `esp32s3-tirtc-device-monitor-full-v1.9.7.bin` | 从 `0x0` 完整烧录；重置整片 Flash 的本地数据 |
| `esp32s3-tirtc-device-monitor-ota-v1.9.7.bin` | OTA 服务端 app；不能写入 `0x0` |
...
| 项目 | 值 |
| --- | --- |
| 文件 | `esp32s3-tirtc-device-monitor-ota-v1.9.7.bin` |
| 大小 | `7,613,216` bytes（`0x742b20`） |
| SHA-256 | `49349c1e1755ae5265e3ddea6f57166ccf60b90cdca7d1a1a004539f4faf43ff` |
| app 分区 | `7798784` bytes |
| 分区剩余 | `185,568` bytes（`2.38%`） |

正式构建确认 app 分区余量为 `185,568` bytes（`2.38%`）。OTA 服务必须使用该次构建
...
`1.9.7` 是 RTC 双向音频产品，H5 不显示设备 RTC 视频是预期行为。板载摄像头只在联系人
二维码扫描时使用。
...
源码构建后的烧录地址以本次 `build/flasher_args.json` 为准。`1.9.7` 默认构建使用四段：
...
保持擦除态 `0xFF`。不要把旧版本的 `storage.bin` 混入 `1.9.7`。
...
manifest 的 `version` 必须为 `1.9.7`，`size` 和 `sha256` 必须与同一 GitHub Release 的
`release-manifest.json`、`SHA256SUMS.txt` 一致。

正常 OTA 只切换 app 分区，保留 NVS 和 storage。设备已经运行 `1.9.7` 时，服务端应返回
