...
`1.9.7` 的 RTC 产品能力是双向音频。Web IPC、小钛、微信 VoIP 和设备互呼共享同一套物理
...
UI 中的扬声器音量和采集增益按钮只提交请求，不直接访问 Codec、麦克风或 NVS。应用控制
任务统一执行硬件设置与持久化；用户连续点击时，中间值可以合并，只保留最新目标值。远端
音量命令仍按收到的顺序处理，不能与本地 UI 合并后丢掉协议语义。

这种合并只减少过期控制操作，不改变媒体帧、AEC 参考或通话状态。界面先显示目标值时，最终
硬件结果仍以应用快照和失败日志为准。

### 来电铃声所有权

普通设备 `call_incoming` 到达后，由 Device Call 服务启动本地合成铃声。铃声由两组上行
三音短句组成，频率依次为 `659 / 784 / 1047 Hz`，周期 `3.2` 秒，并使用 `8 ms` 起音和
`48 ms` 收音包络。铃声任务栈和 PCM 缓冲使用 PSRAM；接听、拒接、对端取消、用户挂断或
身份重置时停止。铃声不属于远端媒体流，不能进入 AEC/通话 profile 后继续播放，也不能跨
会话留下后台任务。

## 生命周期门禁

1. UI 只投递呼叫请求并展示结构化快照，不直接准备音频或启动 RTC。
2. 应用层先准备音频资源，再把呼叫请求交给 Device Call 服务；音频准备和 RTC 就绪是两个
   独立阶段，任一失败都保留真实错误并由应用层释放已经取得的资源。
3. Device Call 服务等待正式上线，最长 `30` 秒；随后等待 TiRTC ready，最长 `50` 秒，
   每 `500 ms` 推进一次 SDK 准备。发起呼叫前，呼叫请求最多等待 `12` 秒，让已经运行的房间
   恢复、联系人刷新或联系人变更任务结束；超时仍忙则拒绝本次呼叫。呼叫活跃后不再启动新的
   房间恢复或联系人刷新，结束后恢复。
4. 主叫在 `OUTGOING` 阶段最多响铃 `45` 秒；进入 `CONNECTING` 后单独等待 P2P 建连
   `40` 秒。前者使用取消语义，后者使用挂断和 `p2p_error` 语义。
5. 每个业务入口显式设置自己的 media profile、stream 和 owner，不依赖上一个页面的残留状态。
6. 退出时先停止业务生产者和回调，再清空自己的队列、取消订阅、断开连接，最后由应用层释放音频。
7. 连接、回调和后台任务使用 generation 或当前连接检查。接听 worker 在连接提交前、提交中
   和等待连接时都复核 generation，旧会话不能把已经结束的房间或新页面拉回旧状态。
8. 微信 VoIP 与 Device Call 的共享命令按会话 owner 分流；一个业务不能消费另一个业务的
   `0x2000` 或断开事件。
9. 视频类型必须明确拒绝，不能为了“呼叫成功”偷偷改成音频。

## SDK 接收公平性

- 一次 ICE UDP 回调最多处理 `8` 个数据报或 `4 ms`，只有确实触及预算时才让出 `1 ms`。
- 一次 TGTRP 音频抖动轮询最多释放 `4` 个有序项；仍有可用积压时，在 `1 ms` 后继续处理。
- 这些限制只把突发恢复拆到多个调度轮次，不改变排序、缺帧恢复或丢包统计，也不给正常单帧
  流量增加固定延时。
...
静态检查可以证明 owner、产品能力和函数调用关系一致；`1.9.7` 的正式干净构建证明代码能够
编译链接，但当前没有目标板回归。真实铃声听感、回声抑制、弱网听感、双向通话、重复
切换和长稳必须在目标板上分别验证，并保留对应日志和主观听感记录。

当前 Web IPC 和设备互呼的人耳试听仍可感知轻微“沙沙电流声”。线上格式核对与 20,000 次
A-law 编解码自检没有发现异常，说明明显的格式不一致和 Codec 自检不稳定不是现有首要证据；
底噪根因仍未证实，`1.9.7` 不宣称已经修复。
