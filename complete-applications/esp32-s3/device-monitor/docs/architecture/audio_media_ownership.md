...
`1.9.6` 的 RTC 产品能力是双向音频。Web IPC、小钛、微信 VoIP 和设备互呼共享同一套物理
...
3. Device Call 服务等待正式上线，最长 `30` 秒；随后等待 TiRTC ready，最长 `50` 秒，
   ...
   房间恢复或联系人刷新，结束后恢复。
4. 每个业务入口显式设置自己的 media profile、stream 和 owner，不依赖上一个页面的残留状态。
5. 退出时先停止业务生产者和回调，再清空自己的队列、取消订阅、断开连接，最后由应用层释放音频。
6. 连接、回调和后台任务使用 generation 或当前连接检查，旧会话不能把新页面拉回旧状态。
7. 微信 VoIP 与 Device Call 的共享命令按会话 owner 分流；一个业务不能消费另一个业务的
   `0x2000` 或断开事件。
8. 视频类型必须明确拒绝，不能为了“呼叫成功”偷偷改成音频。
