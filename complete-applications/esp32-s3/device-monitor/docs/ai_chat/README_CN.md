...
- 按设备 ID 或备注匹配设备联系人并发起音频呼叫；未提供 `call_type` 时按音频处理。
- 按微信 OpenID 或备注匹配微信联系人，并发起音频呼叫。微信联系人当前不提供在线状态
  查询。
- `video` 和其他非音频类型会返回 `unsupported_call_type`，不会静默切换应用或降级媒体。
...
ASR/TTS 字幕、扬声器连续播放，以及联系人查询和音频呼叫动作的成功、离线、歧义、忙线和
不支持类型分支。
