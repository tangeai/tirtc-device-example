...
从设置页打开网络测试并等待 20 次探测完成。页面分别显示：

- 平均时延：成功回复 RTT 的平均值。
- 抖动：相邻成功 RTT 差值绝对值的平均数。
- 丢包率：发送与接收数量计算出的百分比。

当前页面以平均时延不高于 `120 ms`、抖动不高于 `30 ms` 且零丢包作为“基础网络正常”的
提示条件。它只描述这一小段探测窗口，不等于 RTC 音频一定没有抖动；出现“网络质量波动”
时先复测，再结合业务日志判断。
...
1. 浏览器打开 ThingConnect H5：[https://demo-open.tange-ai.com/](https://demo-open.tange-ai.com/)
2. 进入 IPC 或设备监控入口。
...
查看页的音量 `+/-` 在按下时立即生效，静音仍需完整点击确认。

体验目标是确认麦克风、扬声器、网络和 TiRTC 连接都能跑通。`1.9.7` 不发布或播放 RTC
视频，网页没有设备画面是预期产品行为。退出查看页后，设备应释放 IPC 音频资源，回到可
进入其他应用的状态。

Web IPC 上行使用 `8 kHz / 16 bit / mono / G.711 A-law / 20 ms / 160 bytes`。当前试听仍
可能感知轻微“沙沙电流声”。线格式和 A-law 自检未发现异常，但底噪根因尚未证实；把“连接
成功”和“主观音质”作为两项分别记录。
...
4. 点击联系人名称输入框打开屏幕键盘，输入完成后按 `OK`，再点击保存。更新成功后设备
   刷新服务端权威列表。
5. 用户在设备上选择联系人并发起音频呼叫；当前主动呼叫明确使用体验版
   `wx_version_type=2`。
6. 微信侧响铃。
...
2. 两台设备都保持在线。
3. 在设备呼叫页刷新联系人。需要改名时，点击联系人名称输入框打开屏幕键盘，输入完成后按
   `OK`，再保存名称。
4. 选择一个设备联系人。
5. 选择联系人并发起音频呼叫；本固件只接受 `call_type=audio`。首次完整烧录后，设备会在
   后台准备音频并等待正式上线与 TiRTC 就绪，页面保持可操作，不需要手动启动 RTC。
6. 被叫设备收到来电后会播放本地铃声；点击接听或拒接、主叫取消、任一方挂断后铃声应停止。
7. 页面依次显示正在准备、等待接听、建立连接和通话中，并显示主被叫角色、联系人名称与
   设备 ID。对端取消、拒接、挂断、超时、断开或失败时，结果会短暂保留。
8. 双方进入双向音频通话。音量 `+/-` 在按下时立即生效，静音仍需完整点击确认。
9. 主叫或被叫挂断后，服务端释放房间。

设备呼设备和微信 VoIP 是两条不同链路。设备呼设备使用设备联系人和 `TiRtcConnect`；微信通话使用微信授权联系人和 `TiRtcWhipConnect`。

设备互呼使用与 Web IPC、微信上行一致的 A-law 线上格式，并单独启用有界自适应播放缓冲、
全双工高性能 AEC、100 Hz 高通和更保守的通话 AGC 噪声底线。当前试听仍可能听到轻微
“沙沙电流声”，根因尚未闭环；不要把有声、媒体计数正常或 A-law 自检通过写成底噪已修复。
...
5. 在网络测试页展示平均时延、抖动和丢包率。
6. 进入查看页，用 Web 端连接 IPC，展示设备音频上行和网页按住说话。
7. 进入微信页，用小程序呼叫设备，再从设备呼叫微信联系人。
8. 进入小钛，现场说一句话，展示字幕和语音回复；再查询联系人或发起一次呼叫动作。
9. 如果有第二台设备，演示设备呼设备和被叫铃声。
10. 进入设置页检查 OTA，展示版本信息和升级入口。

设置页只保留 Wi-Fi、网络测试、TiRTC 配置和关于 / OTA 等日常入口。RTC 由应用生命周期
自动维护，不再需要 TiRTC 测试、手动启动 RTC 或 sender audio test。
...
| Web IPC 已连接但没有画面 | `1.9.7` 只启用 RTC 双向音频；摄像头只用于二维码扫描 |
| 网络测试显示质量波动 | 复测并分别记录平均时延、抖动和丢包率；不要用单次结果代替 RTC 业务验证 |
| 来电铃声持续播放 | 检查接听、拒接、取消或挂断事件是否真正到达，保留 `CALL_FLOW` 与 `call_ringtone` 首个异常 |
| Web IPC 或设备互呼有轻微沙沙声 | 当前已知现象，根因尚未证实；把业务、方向、音量和诊断日志与主观听感一起记录 |
...
| 设备呼叫 | 冷启动准备、状态提示、接听、拒接、挂断和双向音频分别符合预期 |
| 触摸交互 | 联系人键盘的 `OK`、音量按下响应和静音点击确认分别符合预期 |
