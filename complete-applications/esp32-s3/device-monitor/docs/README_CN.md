...
当前公开版本是 `1.9.6`。首次完整烧录后的设备呼叫会在后台有界准备，呼叫页提供更清楚的
阶段与结束提示；联系人名称可通过屏幕键盘输入，音量加减在按下时立即生效。RTC 由应用
生命周期自动维护，设置页不再保留手动测试入口。服务发现和业务传输继续使用 HTTPS/MQTTS。
Web IPC 和设备互呼仍可能听到轻微“沙沙电流声”，根因尚未闭环，音频专题保留了当前证据
与排查边界。
