...
当前公开版本是 `1.9.7`。呼叫动作和音量调节由应用任务处理，连续音量/增益请求只保留最新
目标；generation 会阻止已结束呼叫的旧 worker 重新拉起房间。主叫按 `45` 秒响铃与 `40` 秒
P2P 建连分阶段计时，来电使用 `659 / 784 / 1047 Hz` 的移动铃声。服务发现和业务传输继续
使用 HTTPS/MQTTS。`1.9.7` 已完成正式干净构建，但还没有真机回归；Web IPC 和设备互呼
仍可能听到轻微“沙沙电流声”，根因尚未闭环，音频专题保留了证据与排查边界。
